# MetalForge Metrics

*Auto-refreshed by `.agents/scripts/fetch-metrics.cjs` — last run 2026-06-21 19:36 UTC*
*Lookback: last 7 days*

---

## GA4 — Audience
| Metric | Last 7d |
| --- | --- |
| Active users | 70 |
| Sessions | 92 |
| Page views | 225 |
| Engagement rate | 66.30% |
| Avg session (s) | 730 |

### Top pages (by page views, last 7d)
| Page | Views | Users |
| --- | --- | --- |
| / | 49 | 11 |
| /drummers | 25 | 3 |
| /tools | 14 | 2 |
| /drummers/igor-cavalera/licks/igor-cavalera-refuse-resist-double-bass | 9 | 1 |
| /drummer/2 | 6 | 6 |
| /drummers/igor-cavalera/licks/igor-cavalera-roots-bloody-roots-tribal-groove | 6 | 1 |
| /tools/setup-builder | 6 | 1 |
| /drummer/joey-jordison | 5 | 4 |
| /drummer/shannon-larkin | 5 | 2 |
| /drummers/igor-cavalera/licks/igor-cavalera-beneath-the-remains-blast | 5 | 1 |

### Traffic sources
| Channel | Sessions | Users |
| --- | --- | --- |
| Organic Search | 50 | 46 |
| Direct | 40 | 22 |
| Unassigned | 8 | 6 |

## Search Console — Search performance
| Metric | Last 7d |
| --- | --- |
| Impressions | 1,439 |
| Clicks | 32 |
| CTR | 2.22% |
| Avg position | 8.7 |

### Top queries
| Query | Impr | Clicks | CTR | Pos |
| --- | --- | --- | --- | --- |
| joey jordison drum kit for sale | 8 | 2 | 25.00% | 5.9 |
| matt greiner drum setup | 13 | 2 | 15.38% | 7.6 |
| jaska raatikainen drum set | 2 | 1 | 50.00% | 9.5 |
| joey jordison drum kit | 74 | 1 | 1.35% | 8.6 |
| mario duplantier gear | 5 | 1 | 20.00% | 5.8 |
| matt garstka drum kit | 6 | 1 | 16.67% | 6.7 |
| matt greiner drum kit | 1 | 1 | 100.00% | 6.0 |
| aquiles priester drum kit | 16 | 0 | 0.00% | 6.8 |
| aquiles priester drum set | 1 | 0 | 0.00% | 12.0 |
| behemoth drum kit | 1 | 0 | 0.00% | 12.0 |

### 🎯 Content-gap queries (impr ≥50, CTR <2%) — CEO MUST address
| Query | Impr | CTR | Pos | Action |
| --- | --- | --- | --- | --- |
| joey jordison drum kit | 74 | 1.35% | 8.6 | open issue to answer better |

---
*Re-run manually: `node .agents/scripts/fetch-metrics.cjs`*
