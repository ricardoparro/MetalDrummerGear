# MetalForge Metrics

*Auto-refreshed by `.agents/scripts/fetch-metrics.cjs` — last run 2026-08-20 19:34 UTC*
*Lookback: last 7 days*

---

## GA4 — Audience
| Metric | Last 7d |
| --- | --- |
| Active users | 236 |
| Sessions | 293 |
| Page views | 557 |
| Engagement rate | 55.63% |
| Avg session (s) | 215 |

### Top pages (by page views, last 7d)
| Page | Views | Users |
| --- | --- | --- |
| /drummers | 62 | 9 |
| / | 47 | 34 |
| /articles/obsolete-drum-setup | 23 | 1 |
| /gear | 13 | 4 |
| /drummer/danny-carey | 12 | 8 |
| /drummer/mario-duplantier | 12 | 10 |
| /drummer/brann-dailor | 10 | 4 |
| /guides | 10 | 3 |
| /drummers/dave-lombardo/evolution | 8 | 2 |
| /tools | 8 | 4 |

### Traffic sources
| Channel | Sessions | Users |
| --- | --- | --- |
| Organic Search | 233 | 178 |
| Direct | 55 | 52 |
| Unassigned | 24 | 23 |
| AI Assistant | 1 | 1 |

### Top countries (sessions, last 7d)
| Country | Sessions | Users |
| --- | --- | --- |
| United States | 118 | 86 |
| Germany | 17 | 15 |
| Canada | 15 | 11 |
| United Kingdom | 13 | 12 |
| France | 12 | 7 |
| Brazil | 10 | 8 |
| Finland | 9 | 6 |
| Singapore | 7 | 7 |
| Australia | 6 | 6 |
| Colombia | 6 | 5 |

## Search Console — Search performance
| Metric | Last 7d |
| --- | --- |
| Impressions | 6,353 |
| Clicks | 137 |
| CTR | 2.16% |
| Avg position | 10.1 |

### Top queries
| Query | Impr | Clicks | CTR | Pos |
| --- | --- | --- | --- | --- |
| best metal drummers of all time | 23 | 2 | 8.70% | 8.1 |
| brann dailor drum kit | 19 | 2 | 10.53% | 6.8 |
| jaska raatikainen drum set | 8 | 2 | 25.00% | 4.3 |
| mario duplantier drum kit | 48 | 2 | 4.17% | 6.6 |
| best metal drum kit | 1 | 1 | 100.00% | 21.0 |
| danny carey drum kit setup | 2 | 1 | 50.00% | 9.0 |
| danny carey drum setup | 13 | 1 | 7.69% | 9.0 |
| eloy casagrande drum kit | 15 | 1 | 6.67% | 9.2 |
| joey jordison drum kit | 68 | 1 | 1.47% | 9.8 |
| joey jordison drum set 1999 | 2 | 1 | 50.00% | 6.0 |

### Top countries (impressions, last 7d)
| Country | Impr | Clicks | CTR | Pos |
| --- | --- | --- | --- | --- |
| usa | 2,391 | 53 | 2.22% | 9.7 |
| can | 293 | 7 | 2.39% | 9.3 |
| deu | 213 | 6 | 2.82% | 7.7 |
| fin | 102 | 6 | 5.88% | 6.7 |
| bra | 182 | 5 | 2.75% | 7.3 |
| fra | 132 | 5 | 3.79% | 9.0 |
| rus | 68 | 5 | 7.35% | 10.3 |
| gbr | 405 | 4 | 0.99% | 9.1 |
| aus | 259 | 3 | 1.16% | 8.8 |
| aut | 42 | 3 | 7.14% | 6.9 |

### 🎯 Content-gap queries (impr ≥50, CTR <2%) — CEO MUST address
| Query | Impr | CTR | Pos | Action |
| --- | --- | --- | --- | --- |
| joey jordison drum kit | 68 | 1.47% | 9.8 | open issue to answer better |

---
*Re-run manually: `node .agents/scripts/fetch-metrics.cjs`*
