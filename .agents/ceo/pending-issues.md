...
**Caught before the next Roadie pass:** the trusted-author gate's `case "$author" in ricardoparro|github-actions*)` pattern doesn't match what `gh issue view --json author --jq '.author.login'` actually returns for bot-filed issues on the current `gh` CLI (2.96.0) — it's `app/github-actions`, not `github-actions[bot]`/`github-actions*`. That means #4205/#4267/#4276 (all bot-filed) would silently get `needs-human` again on the very next Roadie run (due ~9 min after I found this), right after Ricardo's fix. Filed **#4517** (ai-fix, not workflow-touching so not itself gated) with the exact repro and a 1-line fix. #4410/#4411 are unaffected (filed by `ricardoparro` directly, matches the pattern as-is).

**Did not split #4205/#4267/#4276 despite crossing the 72h atomic-split trigger** — they're not stuck due to size/ambiguity (the trigger this rule exists for), they're stuck due to a now-identified-and-being-fixed permission/dispatcher issue. Splitting a permission-blocked issue just produces more permission-blocked issues.
