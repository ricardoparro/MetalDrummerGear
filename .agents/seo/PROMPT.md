...
The workflow fires **four times a day** (`0 1,7,13,19` UTC — see `seo-agent.yml`;
throttled from 12×/day to 2×/day on 2026-07-28 because planning runs were consuming
the Claude subscription quota that Roadie needs to ship, then restored to 4×/day on
2026-07-31 once the bank ran dry 3 runs in a row with Roadie's fleet sitting idle —
see #5146). Your output is **gated by the idea bank**, not a fixed quota.
