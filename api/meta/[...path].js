...
// Issue #7529: the mixed default+named import above resolved BEGINNER_GUIDES to
// the module's namespace object (not its default export) at runtime — named-only
// import matches every other data-module import in this file and avoids the bug.
...
export function getMetaForPath(pathname) {
  ...
  if (gearCompareMatch) {
    ...
    return {
      ...
      faqDisplayItems: gearCompareFaqItems,
      articleSchema: JSON.stringify({
        '@context': 'https://schema.org',
        '@graph': [
          {
          ...
          {
            '@type': 'FAQPage',
            mainEntity: gearCompareFaqItems.map(item => ({
              '@type': 'Question',
              name: item.question,
              acceptedAnswer: { '@type': 'Answer', text: item.answer },
            })),
    ...
    return {
      ...
      articleSchema: JSON.stringify({
        ...
        '@graph': [
          ...
          {
            '@type': 'FAQPage',
            mainEntity: licksHubFaqItems.map(item => ({
              '@type': 'Question',
              name: item.question,
              acceptedAnswer: { '@type': 'Answer', text: item.answer },
            })),
  ...
  if (drummerLicksHubMatch) {
      ...
      return {
        ...
        articleSchema: JSON.stringify({
          ...
          '@graph': [
            ...
            {
              '@type': 'FAQPage',
              mainEntity: drummerLicksFaqItems.map(item => ({
                '@type': 'Question',
                name: item.question,
                acceptedAnswer: { '@type': 'Answer', text: item.answer },
              })),
    ...
    if (drummerForHub) {
      ...
      return {
        ...
        speakableSchema: true,
        speakableCssSelector: ['h1', 'h2', 'p'],
        faqDisplayItems: noLicksFaqItems,
        articleSchema: JSON.stringify({
          '@context': 'https://schema.org',
          '@graph': [
            {
            ...
            {
              '@type': 'FAQPage',
              mainEntity: noLicksFaqItems.map(item => ({
                '@type': 'Question',
                name: item.question,
                acceptedAnswer: { '@type': 'Answer', text: item.answer },
              })),
    ...
    if (drummer) {
      ...
      return {
        ...
        ssrLinks: _dedupeSsrLinksByHref([
          ...
          // Issue #6594: /drummers/<slug>/endorsements (71 sitemap URLs) had no
          // crawlable inbound link from anywhere — only reachable via client-side
          // pushState. Gated on hasEndorsementTimeline so we never link to a
          // slug that isn't actually in the sitemap/has no real content.
  ...
  if (gearSeriesDrummersMatch) {
    ...
    if (Array.isArray(seriesDrummers) && seriesDrummers.length > 0) {
      ...
      // Issue #5621 (split 3/3 of #5521): plain-text mirror of faqMainEntity so
      // non-JS crawlers see the FAQ as visible body text, not just JSON-LD.
      seriesFaqDisplayItems = faqMainEntity.map(item => ({
        question: item.name,
        answer: item.acceptedAnswer.text,
      }));
      graph.push(
    ...
    return {
      ...
      ...(seriesFaqDisplayItems && seriesFaqDisplayItems.length > 0 ? { faqDisplayItems: seriesFaqDisplayItems } : {}),
      articleSchema: JSON.stringify({
        '@context': 'https://schema.org',
        '@graph': graph,
      }),
  ...
  if (songDetailMatch) {
    ...
    if (song) {
      ...
      return {
        ...
        // Issue #5692: CTR-gap follow-up to #5493 — the title fix alone didn't move
        // CTR, so song.metaDescription now overrides the generic fact-restatement
        // template for pages GSC flagged with 3+ consecutive 0%-CTR weeks.
