...
// on the canonical URL instead of leaving two indexable duplicates.
...
export function getMetaForPath(pathname) {
  ...
  // Quiz page
  if (path === '/quiz') {
    // Issue #5524: hoisted so the same Q&A used in the articleSchema FAQPage
    // node below can also be rendered as visible body text via faqDisplayItems,
    // without emitting a second competing FAQPage JSON-LD.
    const quizFaqItems = [
      {
    ...
    return {
      ...
      articleSchema: JSON.stringify({
        ...
        '@graph': [
          ...
          {
            '@type': 'FAQPage',
            mainEntity: drummersHubFaqItems.map(item => ({
              '@type': 'Question',
              name: item.question,
              acceptedAnswer: { '@type': 'Answer', text: item.answer },
            })),
  ...
  if (vsMatch) {
    ...
    if (drummer1 && drummer2) {
      ...
      return {
        ...
        // Issue #5209: Person entities for both compared drummers, mirroring
        // the /battles/ fix (#4462) — closes the /vs/ Person schema gap.
        personSchema: [
  ...
  if (path === '/tools') {
    ...
    // without emitting a second competing FAQPage JSON-LD.
    const toolsFaqItems = [
      { question: 'What free tools does MetalForge offer for drummers?', answer: 'MetalForge offers 6 free tools: Gear Search (find gear by brand or drummer), Dream Set Builder (design your ideal kit), Kit Builder, Gear Comparison, Drummer Tier List, and Metal Drummer Name Generator. All are free at metalforge.io/tools.' },
      { question: 'How do I compare drummer gear setups online?', answer: "Use MetalForge's Gear Comparison tool at metalforge.io/tools/compare to compare drum kit setups side by side between any two pro metal drummers." },
      { question: 'Is there a metal drummer name generator?', answer: "Yes — MetalForge's Metal Drummer Name Generator at metalforge.io/tools/metal-drummer-name-generator creates unique metal drummer aliases using genre, style, and influence inputs." },
    ];
    return {
      ...
      type: 'website',
      url: `${BASE_URL}/tools`,
      // Issue #4925: crawlable links to every tool sub-page — missed from
      // #4355's 9-hub ssrLinks sweep, leaving /tools with a bare bot-facing shell.
      ssrLinks: [
  ...
  if (articlesMatch) {
    ...
    // Issue #5109: legacy pre-rename slugs served byte-identical duplicate
    // title/meta to their canonical whats-in-*-kit counterparts. 301 instead
    // of serving the duplicate, before the ARTICLE_METADATA lookup runs.
    const legacyRedirectSlug = LEGACY_ARTICLE_SLUG_REDIRECTS[articleSlug];
    if (legacyRedirectSlug) {
      return { redirect: `/articles/${legacyRedirectSlug}` };
    }
          ...
          '@graph': [
            {
              ...
              articleSection: top10Article.articleSection || 'Drummer Gear',
              inLanguage: 'en-US',
              keywords: top10Article.seoKeywords?.join(', ') || '',
              author: {
                '@type': 'Organization',
                name: top10Article.author || 'MetalForge',
                url: BASE_URL,
            ...
            {
              '@type': 'FAQPage',
              mainEntity: top10ArticleFaqItems.map(item => ({
                '@type': 'Question',
                name: item.question,
                acceptedAnswer: { '@type': 'Answer', text: item.answer },
              })),
  ...
  if (genreMatch) {
    ...
    if (genreData) {
      ...
      return {
        ...
        speakableSchema: true,
        speakableCssSelector: ['h1', 'h2', 'p'],
        // Issue #5524: same faqItems already embedded in articleSchema's FAQPage
        // above, mirrored as visible body text — no second FAQPage JSON-LD.
        faqDisplayItems: faqItems,
      };
    }
  }
  ...
  if (path === '/licks') {
    ...
    // node below can also be rendered as visible body text via faqDisplayItems,
    // without emitting a second competing FAQPage JSON-LD.
    const LICKS_HUB_FAQ_ITEMS = [
      { question: 'What are the best metal drum licks to learn?', answer: 'MetalForge features signature drum licks from 63 metal legends including blast beats from George Kollias, double-bass patterns from Joey Jordison, and groove licks from Brann Dailor. Browse by drummer at metalforge.io/licks.' },
      { question: 'How many signature drum licks does MetalForge have?', answer: 'MetalForge has 157 signature lick pages covering 63 professional metal drummers, with step-by-step HowTo breakdowns and video analysis for each lick.' },
      { question: 'What technique is most common in metal drum licks?', answer: 'Blast beats and double-bass patterns are the most common techniques in metal drum licks. Explore technique-specific licks at metalforge.io/technique/blast-beat/drummers and metalforge.io/technique/double-bass/drummers.' },
    ];
    return {
      ...
      articleSchema: JSON.stringify({
        ...
        '@graph': [
          ...
          {
            '@type': 'FAQPage',
            mainEntity: LICKS_HUB_FAQ_ITEMS.map(item => ({
              '@type': 'Question',
              name: item.question,
              acceptedAnswer: { '@type': 'Answer', text: item.answer },
            })),
      ...
      // Issue #5524: same LICKS_HUB_FAQ_ITEMS already embedded in articleSchema's
      // FAQPage above, mirrored as visible body text — no second FAQPage JSON-LD.
      faqDisplayItems: LICKS_HUB_FAQ_ITEMS,
    };
  }
  ...
  if (drummerLicksHubMatch) {
      ...
      return {
        ...
        articleSchema: JSON.stringify({
          ...
          '@graph': [
            ...
            {
              '@type': 'FAQPage',
              mainEntity: drummerLicksHubFaqItems.map(item => ({
                '@type': 'Question',
                name: item.question,
                acceptedAnswer: { '@type': 'Answer', text: item.answer },
              })),
        ...
        faqDisplayItems: drummerLicksHubFaqItems,
      };
    }
    ...
    if (drummerForHub) {
      ...
      // node below can also be rendered as visible body text via faqDisplayItems,
      // without emitting a second competing FAQPage JSON-LD.
      const drummerLicksHubNoLicksFaqItems = [
        {
      ...
      return {
        ...
        articleSchema: JSON.stringify({
          ...
          '@graph': [
            ...
            {
              '@type': 'FAQPage',
              mainEntity: drummerLicksHubNoLicksFaqItems.map(item => ({
                '@type': 'Question',
                name: item.question,
                acceptedAnswer: { '@type': 'Answer', text: item.answer },
              })),
        ...
        faqDisplayItems: drummerLicksHubNoLicksFaqItems,
      };
    }
  }
          ...
          '@graph': [
            {
              ...
              dateModified: listDateModified,
              isAccessibleForFree: true,
              articleSection: list.articleSection || 'Drummer Gear',
              inLanguage: 'en-US',
  ...
  // plural-path routes (gearHistoryMatch/evolutionMatch/endorsementsMatch
  // above) were falling through to the generic drummerCategoryMatch shell
  // below on the singular /drummer/ path, since none of them are in
  // DRUMMER_GEAR_CATEGORIES. Mirrors each plural handler's content/schema
  // exactly, only the URLs are singular.
  ...
  const drummerEvolutionMatch = path.match(/^\/drummer\/([^/]+)\/evolution\/?$/);
    ...
    if (drummer) {
      ...
      return {
        ...
        breadcrumbSchema: [
          ...
          { name: 'Drummers', url: `${BASE_URL}/drummers` },
          { name: drummer.name, url: `${BASE_URL}/drummer/${slug}` },
          { name: 'Endorsements', url: `${BASE_URL}/drummer/${slug}/endorsements` },
        ],
        speakableSchema: true,
        speakableCssSelector: ['h1', 'h2', 'p'],
        faqSchema: [
  ...
  // Issue #5000: /drummer/<slug>/licks singular-path — same regex-collision
  // class as #4963 (bio) and #4982 (gear-history/evolution/endorsements):
  // the real, already-built drummerLicksHubMatch handler above only matches
  // the plural /drummers/ path, so bots/LLMs guessing the singular form (by
  // analogy with the /drummer/<slug> profile page) fell through to the
  // generic drummerCategoryMatch shell below, since 'licks' is not in
  // DRUMMER_GEAR_CATEGORIES. Mirrors the plural handler's content/schema
  // exactly, only the URLs are singular.
    ...
    return {
      ...
      articleSchema: JSON.stringify([
        ...
        },
        {
          '@context': 'https://schema.org',
          '@type': 'FAQPage',
          mainEntity: historyFaqItems.map(item => ({
            '@type': 'Question',
            name: item.question,
            acceptedAnswer: { '@type': 'Answer', text: item.answer },
          })),
  ...
  if (path === '/battles') {
    ...
    // node below can also be rendered as visible body text via faqDisplayItems,
    // without emitting a second competing FAQPage JSON-LD.
    const battlesHubFaqItems = [
      { question: 'Who is the best metal drummer?', answer: 'MetalForge lets fans vote on legendary matchups including Lars Ulrich vs Dave Lombardo, Gene Hoglan vs George Kollias, and more.' },
      { question: 'How do MetalForge drummer battles work?', answer: "Each week a new matchup is featured. Vote for your favorite, see the community results, and explore each drummer's full gear setup." },
      { question: 'How many drummer battles are on MetalForge?', answer: 'MetalForge features 8 curated head-to-head drummer battles spanning thrash, death, prog, and black metal.' },
      { question: 'How is the winner of a MetalForge drummer battle decided?', answer: 'Winners are decided by community voting — fans vote for their favorite drummer in each matchup and results update live.' },
    ];
    return {
      ...
      articleSchema: JSON.stringify({
        ...
        '@graph': [
          ...
          {
            '@type': 'FAQPage',
            mainEntity: battlesHubFaqItems.map(item => ({
              '@type': 'Question',
              name: item.question,
              acceptedAnswer: { '@type': 'Answer', text: item.answer },
            })),
      ...
      faqDisplayItems: battlesHubFaqItems,
    };
  }
  ...
  if (drummerPrefixMatch) {
    ...
    if (drummer) {
      ...
      // Issue #5520: plain-text mirror of faqMainEntity so non-JS crawlers see
      // the curated FAQ as visible body text, not just JSON-LD.
      ...
      return {
        ...
        url: `${BASE_URL}/drummer/${slug}`,
        ssrLinks: _dedupeSsrLinksByHref([
          // Issue #5479: inbound links from the profile shell into its own
          // gear-category subpages — only for categories with real data, so we
          // never link to a knowably-empty page.
  ...
  if (cymbalBrandMatch) {
    ...
    if (brand) {
      ...
      const studyLinks = getBrandStudyLinks(brand.name);
      const brandDrummers = _brandConfirmedDrummers(confirmedSetups);
      return {
        ...
        // Issue #5268: Person schema for this brand's confirmed drummers —
        // mirrors #5228 (drumsticks/snares); was the one remaining sibling
        // never wired to _brandConfirmedDrummers.
        ...(brandDrummers.length > 0 ? {
          personSchema: brandDrummers.map(d => ({
            name: d.name,
            url: `${BASE_URL}/drummer/${d.slug}`,
  ...
  if (cymbalSetupMatch) {
    ...
    if (data) {
      return {
        ...
        type: 'article',
        url: data.canonicalUrl,
        // Issue #5223: Person entity for the page's drummer, mirroring the
        // /vs/ fix (#5209) — closes the /cymbals/setups/ Person schema gap.
        personSchema: [
          { name: drummer.name, url: `${BASE_URL}/drummer/${slug}`, band: drummer.band },
        ],
  ...
  if (pedalSetupMatch) {
    ...
    if (data) {
      return {
        ...
        type: 'article',
        url: data.canonicalUrl,
        // Issue #5224: Person entity for the page's drummer, mirroring the
        // /vs/ fix (#5209) — closes the /pedals/setups/ Person schema gap.
        personSchema: [
          { name: drummer.name, url: `${BASE_URL}/drummer/${slug}`, band: drummer.band },
        ],
        // Issue #5024: the BreadcrumbList in articleSchema below is JSON-LD
...
export default function handler(req, res) {
  ...
  // Issue #5109: legacy article slugs 301 to their canonical counterpart
  // instead of serving duplicate title/meta.
  if (meta.redirect) {
