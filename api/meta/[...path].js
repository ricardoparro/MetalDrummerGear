...
export function getMetaForPath(pathname) {
  ...
  if (listMatch) {
    ...
    if (list) {
      ...
      // Issue #5522: hoisted so the same Q&A array backs both the FAQPage node
      // inside articleSchema below and meta.faqDisplayItems (visible body text,
      // no JSON-LD — avoids a duplicate FAQPage block, see #5520's mechanism).
      ...
      return {
        ...
        // Issue #5522: plain-text mirror of listFaqItems (visible body only, no
        // JSON-LD — the FAQPage node above already covers structured data).
