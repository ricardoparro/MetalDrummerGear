...
// ==========================================
function BandLinksSection({ drummer, theme }) {
  const { width } = useWindowDimensions();
  const isMobile = width < 768;
  
  // Get bands for this drummer by ID
  const bands = drummer?.id ? getBandsForDrummer(drummer.id) : [];
  ...
  const handleBandPress = (bandSlug) => {
    if (Platform.OS === 'web' && typeof window !== 'undefined') {
      window.history.pushState({}, '', `/band/${bandSlug}`);
          ...
          return Platform.OS === 'web' ? (
            ...
            >
              <View style={[styles.bandLinkCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
                ...
                <Text style={[styles.bandLinkName, { color: theme.text }]}>{band.name}</Text>
              </View>
            </a>
          ) : (
            <TouchableOpacity
