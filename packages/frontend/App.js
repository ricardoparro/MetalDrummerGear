...
function ImageWithFallback({ source, style, accessibilityLabel, priority = false, width, height }) {
  ...
  // Use width hint for responsive sizing (reduces bandwidth)
  const optimizedUri = useMemo(() => {
    const uri = source?.uri;
    if (!uri) return PLACEHOLDER_IMAGE;
    // For local images or already-optimized URLs, use as-is
    if (uri.startsWith('/images/') || uri.startsWith('/api/image')) {
      return uri;
    }
    // For external images, proxy through our optimizer
    // Use the display width to request appropriately sized images
    const targetWidth = width || 256; // Default to medium size
