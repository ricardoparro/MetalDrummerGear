...
// Browse all drummer quotes with filters
// ==========================================
function QuotesPage({ theme, onBack, drummers, onSelectDrummer }) {
  ...
  // Filter states
  const [selectedDrummerId, setSelectedDrummerId] = useState('');
  ...
  const filteredQuotes = useMemo(() => {
    ...
    if (selectedDrummerId) {
      result = result.filter(q => q.drummerId.toString() === selectedDrummerId);
    }
    if (selectedTopic) {
      result = result.filter(q => q.topic === selectedTopic);
    }
    return result;
  }, [allQuotes, selectedDrummerId, selectedTopic]);
  ...
  useEffect(() => {
    if (Platform.OS === 'web' && typeof document !== 'undefined') {
      document.title = 'Drummer Quotes - Metal Drummer Gear';
      const setMeta = (name, content, isProperty = false) => {
        const attr = isProperty ? 'property' : 'name';
        let meta = document.querySelector(`meta[${attr}="${name}"]`);
        if (!meta) {
      ...
      setMeta('description', 'Inspiring quotes from legendary metal drummers including Lars Ulrich, Joey Jordison, Dave Lombardo, and more.');
      setMeta('og:title', 'Drummer Quotes - Metal Drummer Gear', true);
      setMeta('og:description', 'Inspiring quotes from legendary metal drummers', true);
      
      // Add schema markup for quotes
      let quotesSchema = document.querySelector('script[data-schema="quotes-page"]');
      if (!quotesSchema) {
        quotesSchema = document.createElement('script');
        quotesSchema.type = 'application/ld+json';
        quotesSchema.setAttribute('data-schema', 'quotes-page');
        document.head.appendChild(quotesSchema);
      }
      const schemaData = allQuotes.slice(0, 10).map(q => ({
        "@type": "Quotation",
        "text": q.text,
        "spokenByCharacter": { "@type": "Person", "name": q.drummerName },
        "citation": q.source
      }));
      quotesSchema.textContent = JSON.stringify({
        "@context": "https://schema.org",
        "@type": "ItemList",
  ...
  const clearFilters = () => {
    setSelectedDrummerId('');
    setSelectedTopic('');
  };
  
  const hasFilters = selectedDrummerId || selectedTopic;
  ...
  return (
    <ScrollView style={[styles.detailContainer, { backgroundColor: theme.background }]} contentContainerStyle={styles.detailContent}>
      ...
      <Text style={[styles.compareSubtitle, { color: theme.secondaryText }]}>
        Wisdom and insights from legendary metal drummers
      </Text>
      
      {/* Random Quote Button */}
      <TouchableOpacity
        onPress={getRandomQuote}
        style={[styles.quizButton, { backgroundColor: '#dc2626', marginHorizontal: 0, marginBottom: 20 }]}
        accessibilityRole="button"
        accessibilityLabel="Get random quote"
      >
        <Text style={[styles.quizButtonText, { color: '#fff' }]}>🎲 Random Quote</Text>
      </TouchableOpacity>
      
      {/* Random Quote Display */}
      {randomQuote && (
        <View style={[styles.section, { backgroundColor: theme.card, borderColor: '#dc2626', borderWidth: 2, marginBottom: 20 }]}>
          <Text style={[styles.quoteText, { color: theme.text, fontSize: 18, textAlign: 'center' }]}>
            "{randomQuote.text}"
          </Text>
          <TouchableOpacity onPress={() => onSelectDrummer(randomQuote.drummerId)}>
            <Text style={[styles.quoteSource, { color: '#dc2626', textAlign: 'center', fontSize: 15 }]}>
              — {randomQuote.drummerName} ({randomQuote.drummerBand})
            </Text>
          </TouchableOpacity>
          <Text style={[styles.quoteSource, { color: theme.secondaryText, textAlign: 'center', marginTop: 4 }]}>
            {randomQuote.source}{randomQuote.year ? ` (${randomQuote.year})` : ''}
          </Text>
          <TouchableOpacity
            onPress={() => setRandomQuote(null)}
            style={{ position: 'absolute', top: 10, right: 10, padding: 4 }}
          >
            <Text style={{ color: theme.secondaryText, fontSize: 18 }}>✕</Text>
          </TouchableOpacity>
        </View>
      )}
      
      {/* Filters */}
      <View style={[styles.section, { backgroundColor: theme.card, borderColor: theme.border, marginBottom: 20 }]}>
        <Text style={[styles.sectionTitle, { color: theme.text }]}>Filter Quotes</Text>
        
        <View style={{ flexDirection: isMobile ? 'column' : 'row', gap: 16 }}>
          {/* Drummer Filter */}
          <View style={{ flex: 1, minWidth: 150 }}>
            <Text style={{ color: theme.secondaryText, fontSize: 12, marginBottom: 6 }}>By Drummer</Text>
            {Platform.OS === 'web' ? (
              <select
                value={selectedDrummerId}
                onChange={(e) => setSelectedDrummerId(e.target.value)}
                style={{
                  width: '100%',
                  padding: '10px 12px',
                  fontSize: 14,
              ...
              >
                <option value="">All Drummers</option>
                {drummersWithQuotes.map(d => (
                  <option key={d.id} value={d.id.toString()}>{d.name} ({d.band})</option>
                ))}
              </select>
            ) : (
              <Text style={{ color: theme.secondaryText }}>Filter available on web</Text>
            )}
          </View>
          
          {/* Topic Filter */}
          <View style={{ flex: 1, minWidth: 150 }}>
            <Text style={{ color: theme.secondaryText, fontSize: 12, marginBottom: 6 }}>By Topic</Text>
            {Platform.OS === 'web' ? (
              <select
                value={selectedTopic}
        ...
        {hasFilters && (
          <TouchableOpacity onPress={clearFilters} style={{ marginTop: 12, alignSelf: 'flex-start' }}>
            <Text style={{ color: theme.error || '#ef4444', fontSize: 14, fontWeight: '500' }}>Clear Filters</Text>
          </TouchableOpacity>
        )}
        
        <Text style={{ color: theme.secondaryText, fontSize: 13, marginTop: 12 }}>
      ...
      <View style={{ gap: 16 }}>
        {filteredQuotes.map((quote, index) => (
          <View 
            ...
            style={[styles.section, { backgroundColor: theme.card, borderColor: theme.border, marginBottom: 0 }]}
          >
            {/* Quote Text */}
            <Text style={[styles.quoteText, { color: theme.text }]}>
              "{quote.text}"
            ...
            {/* Attribution */}
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 12, marginTop: 8 }}>
              <TouchableOpacity 
              ...
              <View style={{ alignItems: 'flex-end' }}>
                {quote.topic && (
                  <View style={{ backgroundColor: theme.border, paddingVertical: 4, paddingHorizontal: 10, borderRadius: 12, marginBottom: 4 }}>
                    <Text style={{ color: theme.text, fontSize: 11, fontWeight: '500', textTransform: 'capitalize' }}>
                      {quote.topic}
                    </Text>
                  </View>
                )}
        ...
        {filteredQuotes.length === 0 && (
          <View style={{ padding: 40, alignItems: 'center' }}>
            <Text style={{ color: theme.secondaryText, fontSize: 16, textAlign: 'center', marginBottom: 12 }}>
              No quotes found matching your criteria.
            </Text>
            <TouchableOpacity onPress={clearFilters}>
              <Text style={{ color: '#dc2626', fontSize: 14, fontWeight: '500' }}>Clear Filters</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
