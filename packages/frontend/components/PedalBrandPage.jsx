...
// Renders /pedals/brands/<brand>: positioning, metal-relevant lines, and
...
function DrummerPedalRow({ pedal, drummer, theme, onNavigateToDrummer }) {
  return (
    <CrawlableLink
      href={`/drummer/${pedal.drummerSlug}`}
      onPress={onNavigateToDrummer ? () => onNavigateToDrummer(pedal.drummerSlug) : undefined}
    ...
    >
      <View style={[styles.drummerRow, { borderColor: theme.border }]}>
        ...
        <Text style={[styles.drummerRowPedal, { color: theme.secondaryText || theme.text }]} numberOfLines={2}>
          {pedal.summary}
        </Text>
      </View>
    </CrawlableLink>
...
export function PedalBrandPage({
  ...
  useEffect(() => {
    ...
    updateOgMeta({
      ...
      keywords: `${brand.name.toLowerCase()} pedals, ${brand.name.toLowerCase()} bass drum pedal for metal`,
      url,
      type: 'article',
    });
  ...
  if (!brand) {
    return (
      <View style={[styles.container, styles.centered, { backgroundColor: theme.background }]}>
        ...
        {onNavigateBrandsHub && (
          <CrawlableLink href="/pedals/brands" onPress={onNavigateBrandsHub}>
            <Text style={[styles.backLink, { color: theme.primary || theme.text }]}>&larr; All pedal brands</Text>
          </CrawlableLink>
        )}
      </View>
    );
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.background }]} contentContainerStyle={styles.content}>
      <View style={styles.breadcrumbs}>
        ...
        <CrawlableLink href="/pedals/brands" onPress={onNavigateBrandsHub} style={{ marginRight: 4 }}>
      ...
      <Text style={[styles.h2, { color: theme.text }]}>Metal-relevant lines</Text>
      ...
      {confirmedPedals.length > 0 ? (
        ...
        <Text style={[styles.body, { color: theme.secondaryText || theme.text }]}>
          We haven't verified a {brand.name} pedal setup played by one of our mapped metal drummers yet —
      ...
      <CrawlableLink href="/pedals/brands" onPress={onNavigateBrandsHub} style={{ marginTop: 12 }}>
        <View style={[styles.linkCard, { backgroundColor: theme.cardBg || theme.background, borderColor: theme.border }]}>
          <Text style={[styles.linkCardText, { color: theme.text }]}>🦶 All Pedal Brands →</Text>
        </View>
      </CrawlableLink>
