// Pedal brands hub — Issue #4432 (split 1/3 of #4394)
// Renders /pedals/brands: one card per major pedal brand, linking to each
// brand's detail page at /pedals/brands/<brand>.
...
function BrandCard({ brand, theme, onNavigate }) {
  const confirmedCount = getConfirmedPedalsForBrand(brand).length;
  return (
...
export function PedalBrandsHubPage({ theme: themeProp, onNavigateBrand, onBack }) {
  ...
  useEffect(() => {
    ...
    updateOgMeta({
      title: 'Pedal Brands: Tama, Pearl, DW, Axis & Trick | MetalForge',
      description: 'Positioning, metal-relevant lines, and confirmed drummer usage for the five bass drum pedal brands metal drummers actually play.',
      keywords: 'pedal brands, tama pedals, pearl pedals, dw pedals, axis pedals, trick pedals',
      url,
      type: 'website',
    });
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.background }]} contentContainerStyle={styles.content}>
      ...
      <Text style={[styles.intro, { color: theme.text }]}>
        Five major bass drum pedal makers — their positioning, metal-relevant lines, and which drummers in our
