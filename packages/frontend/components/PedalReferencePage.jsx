// Pedal reference pages — Issue #4392 (epic #4387 phase 2/4)
...
// shape: intro sections, a spec/trade-off table with a cited source, and an
// FAQ block — mirrors components/SnareReferencePage.jsx.
    ...
    <View style={[styles.table, { borderColor: theme.border }]}>
      ...
      {page.table.map((row) => (
        <View key={row.driveType} style={[styles.tableRow, { borderColor: theme.border }]}>
          ...
          <Text style={[styles.tableCell, styles.colSpec, { color: theme.text }]}>{row.feel}</Text>
      ...
      <Text style={[styles.tableCaveat, { color: theme.secondaryText || theme.text }]}>
        Typical — exact feel varies by brand, model, and maintenance. Worked example sourced from{' '}
...
function SingleVsDoubleTable({ page, theme }) {
  return (
    <View style={[styles.table, { borderColor: theme.border }]}>
      <View style={[styles.tableRow, styles.tableHeaderRow, { borderColor: theme.border }]}>
        <Text style={[styles.tableCellHeader, styles.colSize, { color: theme.text }]}>Setup</Text>
        <Text style={[styles.tableCellHeader, styles.colSpec, { color: theme.text }]}>Mechanism</Text>
        <Text style={[styles.tableCellHeader, styles.colUse, { color: theme.text }]}>Role in metal</Text>
      </View>
      {page.table.map((row) => (
        <View key={row.setup} style={[styles.tableRow, { borderColor: theme.border }]}>
          <Text style={[styles.tableCell, styles.colSize, { color: theme.text, fontWeight: '700' }]}>{row.setup}</Text>
          <Text style={[styles.tableCell, styles.colSpec, { color: theme.text }]}>{row.mechanism}</Text>
          <Text style={[styles.tableCell, styles.colUse, { color: theme.secondaryText || theme.text }]}>{row.metalRole}</Text>
        </View>
      ))}
      <Text style={[styles.tableCaveat, { color: theme.secondaryText || theme.text }]}>
        Typical — exact terminology and setup vary by drummer. Worked example sourced from{' '}
        {page.tableSource.label}.
      </Text>
    </View>
  );
  ...
  return (
    <View style={[styles.table, { borderColor: theme.border }]}>
      <View style={[styles.tableRow, styles.tableHeaderRow, { borderColor: theme.border }]}>
        ...
        <Text style={[styles.tableCellHeader, styles.colSpec, { color: theme.text }]}>Tone / attack</Text>
        <Text style={[styles.tableCellHeader, styles.colUse, { color: theme.text }]}>Role in metal</Text>
      </View>
      {page.table.map((row) => (
      ...
      <Text style={[styles.tableCaveat, { color: theme.secondaryText || theme.text }]}>
        Typical — exact attack varies by brand, felt density, and beater angle. Worked example sourced from{' '}
        {page.tableSource.label}.
      </Text>
    </View>
  );
...
export function PedalReferencePage({ theme: themeProp, slug, onBack, onNavigateReference }) {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.background }]} contentContainerStyle={styles.content}>
      ...
      {slug === 'single-vs-double' && page.table && <SingleVsDoubleTable page={page} theme={theme} />}
