...
// Renders /pedals: why the pedal is a metal drummer's most personal gear
// choice, how to choose one, links to the reference pages
// (drive-types/single-vs-double/setup-tuning), brand overview, and the
// verified signature pedals from data/pedals.js (phase 1).
...
function SignaturePedalCard({ drummer, pedal, theme }) {
  ...
  const anchorText = `${drummer.name}'s ${pedal.brand ? `${pedal.brand} ` : ''}pedal`;
  const card = (
...
export function PedalsHubPage({ theme: themeProp, drummers = [], onNavigateReference, onNavigateBestForMetal }) {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.background }]} contentContainerStyle={styles.content}>
      ...
      <Text style={[styles.h2, { color: theme.text }]}>How to choose a pedal</Text>
      ...
      {signaturePedals.length > 0 && (
        <>
          <Text style={[styles.h2, { color: theme.text }]}>Verified drummer pedals</Text>
