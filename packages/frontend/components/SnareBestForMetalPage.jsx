...
// Renders /snares/best-for-metal: why metal favours high-tension metal
// shells (with the wood-shell exceptions), size/tuning notes, then the
// verified signature snares pulled live from data/snares.js. Affiliate-CTA-
// ready — each recommendation card renders retailer buttons automatically
// once retailerUrls is populated, and falls back to a link to the drummer's
// signature snare page until then.
...
// retailerUrls has entries; until then, falls back to the signature snare
// page so the card is never a dead end.
function SnareCta({ snare, theme, onNavigateToSignature }) {
  ...
  if (retailerEntries.length === 0) {
    return (
      <CrawlableLink
        ...
        onPress={onNavigateToSignature ? () => onNavigateToSignature(snare.drummerSlug) : undefined}
      >
...
function RecommendationCard({ snare, drummer, theme, onNavigateToSignature }) {
  return (
    <View style={[styles.card, { backgroundColor: theme.cardBg || theme.background, borderColor: theme.border }]}>
      <Text style={[styles.cardTitle, { color: theme.text }]}>{snare.brand} {snare.model}</Text>
      ...
      <SnareCta snare={snare} theme={theme} onNavigateToSignature={onNavigateToSignature} />
    </View>
  );
}

export function SnareBestForMetalPage({ theme: themeProp, drummers = [], onBack, onNavigateToSignature }) {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.background }]} contentContainerStyle={styles.content}>
      ...
      {[page.whyMetalShells, page.woodExceptions, page.sizeAndTuning].map((section) => (
        ...
        {SIGNATURE_SNARES.map((snare) => {
          ...
          return (
            <View key={snare.drummerSlug} style={styles.cell}>
              <RecommendationCard snare={snare} drummer={drummer} theme={theme} onNavigateToSignature={onNavigateToSignature} />
            </View>
          );
        })}
      </View>
