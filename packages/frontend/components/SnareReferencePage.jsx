// Snare reference pages — Issue #4310 (epic #4308 phase 2/4)
// Renders /snares/shells, /snares/sizes, /snares/tuning-for-metal.
...
// source, and an FAQ block — mirrors components/CymbalReferencePage.jsx.
...
function ShellsTable({ page, theme }) {
  return (
    <View style={[styles.table, { borderColor: theme.border }]}>
      <View style={[styles.tableRow, styles.tableHeaderRow, { borderColor: theme.border }]}>
        <Text style={[styles.tableCellHeader, styles.colSize, { color: theme.text }]}>Material</Text>
      ...
      {page.table.map((row) => (
        <View key={row.material} style={[styles.tableRow, { borderColor: theme.border }]}>
          <Text style={[styles.tableCell, styles.colSize, { color: theme.text, fontWeight: '700' }]}>
            {row.material} <Text style={{ fontWeight: '400', fontStyle: 'italic' }}>({row.category})</Text>
          </Text>
      ...
      <Text style={[styles.tableCaveat, { color: theme.secondaryText || theme.text }]}>
        Typical — exact tone varies by brand, model, and construction (cast/rolled/hand-spun). Worked example sourced from{' '}
  ...
  return (
    <View style={[styles.table, { borderColor: theme.border }]}>
      <View style={[styles.tableRow, styles.tableHeaderRow, { borderColor: theme.border }]}>
        ...
        <Text style={[styles.tableCellHeader, styles.colSpec, { color: theme.text }]}>Category</Text>
      ...
      {page.table.map((row) => (
        <View key={row.size} style={[styles.tableRow, { borderColor: theme.border }]}>
          ...
          <Text style={[styles.tableCell, styles.colSpec, { color: theme.text }]}>{row.category}</Text>
          <Text style={[styles.tableCell, styles.colUse, { color: theme.secondaryText || theme.text }]}>{row.why}</Text>
        </View>
      ))}
      <Text style={[styles.tableCaveat, { color: theme.secondaryText || theme.text }]}>
        Typical — exact size is a player's choice and varies by drummer and model. Worked example sourced from{' '}
        {page.tableSource.label}.
      </Text>
    </View>
  );
...
export function SnareReferencePage({ theme: themeProp, slug, onBack, onNavigateReference }) {
  ...
  useEffect(() => {
    ...
    injectSchema(`snare-ref-${page.slug}-article`, generateArticleSchema(page, url));
    injectSchema(`snare-ref-${page.slug}-faq`, generateFaqSchema(page.faq));
    return () => {
      removeSchema(`snare-ref-${page.slug}-article`);
      removeSchema(`snare-ref-${page.slug}-faq`);
    };
  }, [slug]);
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.background }]} contentContainerStyle={styles.content}>
      ...
      {slug === 'shells' && page.table && <ShellsTable page={page} theme={theme} />}
      ...
      <View style={styles.otherPages}>
        <Text style={[styles.h2, { color: theme.text }]}>More snare guides</Text>
        {otherPages.map((s) => (
          <CrawlableLink
            key={s}
            href={`/snares/${s}`}
...
export default SnareReferencePage;
