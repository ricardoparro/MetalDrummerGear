// Album drum-setup articles — Matt Garstka. Auto-split from the former monolith.
export const articles = {
  "whats-in-matt-garstkas-kit": {
    "slug": "whats-in-matt-garstkas-kit",
    "articleType": "drummer-kit",
    "drummer": "Matt Garstka",
    "drummerId": 53,
    "relatedDrummerSlug": "matt-garstka",
    "band": "Animals as Leaders",
    "bands": [
      "Animals as Leaders"
    ],
    "genre": "Progressive Metal / Djent",
    "country": "USA",
    "isAlbumArticle": true,
    "datePublished": "2026-03-27",
    "dateModified": "2026-03-27",
    "author": "MetalForge Editorial",
    "title": "What's In Matt Garstka's Kit: The Jazz-Fusion Meets Progressive Metal Arsenal",
    "description": "Discover Matt Garstka's complete drum setup — the DW Collector's Series kit, Meinl Byzance cymbals, and signature gear that power Animals as Leaders. Complete breakdown of the modern prog pioneer's equipment.",
    "seoKeywords": [
      "matt garstka drum kit",
      "matt garstka setup",
      "animals as leaders drummer gear",
      "matt garstka dw drums",
      "matt garstka meinl cymbals",
      "matt garstka signature",
      "djent drums",
      "prog metal drums"
    ],
    "ogImage": "/images/drummers/matt-garstka.webp",
    "intro": {
      "title": "The Academic Approach to Progressive Metal",
      "content": "Matt Garstka represents a new paradigm in heavy music drumming. Where previous generations relied on raw aggression and speed, Garstka brings Berklee-educated jazz sophistication to the world of djent and progressive metal. The result is drumming that's simultaneously technical and deeply musical.\n\nBorn April 27, 1989, in Westfield, Massachusetts, Garstka began drumming at age 5 and was performing professionally by his teens. After studying at Berklee College of Music, he joined Animals as Leaders in 2012, replacing Navene Koperweis. His first album with the band, \"The Joy of Motion\" (2014), immediately established him as one of the most important drummers of his generation.\n\nWhat sets Garstka apart isn't just his technical ability — it's his approach. He uses traditional grip in a genre dominated by matched grip players. He prioritizes dynamics and ghost notes in music that often favors wall-to-wall intensity. He makes 7/8 feel as natural as 4/4.\n\nHis educational presence has made him one of the most studied drummers of the modern era. Through Drumeo, YouTube, and international clinics, Garstka has shared his approach with millions, influencing how a generation thinks about progressive drumming.\n\nThis article breaks down every piece of gear Matt uses to achieve his distinctive sound — from his DW Collector's Series kit, which he switched to in September 2021 after years on Pearl, to his extensive Meinl Byzance cymbal setup.",
      "keyPoints": [
        "Berklee College of Music graduate with jazz fusion background",
        "Joined Animals as Leaders in 2012, transforming their sound",
        "Uses traditional grip — unusual in progressive metal",
        "DW Drums (switched from Pearl in September 2021) and Meinl Cymbals endorsee with signature products",
        "One of the most influential drum educators of his generation"
      ]
    },
    "drumKit": {
      "title": "Matt's DW Collector's Series Setup",
      "brand": "DW",
      "model": "DW Collector's Series (custom shop)",
      "finish": "Custom per DW custom shop order",
      "config": {
        "shells": "Mixed Collector's Series wood species and shell configurations, per DW custom shop"
      },
      "description": "Matt Garstka's drum setup reflects his jazz-influenced approach: smaller, more responsive drums that prioritize articulation over raw power. In September 2021 he switched from the Pearl Masterworks Maple kit he'd played since \"The Joy of Motion\" (2014) to a DW custom shop build. DW's official signing announcement described the kit as mixing Collector's Series wood species and shell configurations, continuing the same compact, four-piece philosophy that forced musical creativity on the earlier Pearl setup.\n\nSpecific shell sizes for the current DW kit have not been independently verified; this article will be updated with exact configuration details as sourcing allows. On the earlier Pearl kit, Garstka favored a notably compact 20\" bass drum with 10\" and 12\" rack toms and a 14\" floor tom — sized for quick attack and clear articulation rather than raw power, essential for the intricate kick and tom patterns in Animals as Leaders' music.\n\nThis minimalist philosophy extends to his entire approach. Rather than adding drums, Matt adds complexity through technique: ghost notes, dynamic variations, polyrhythmic layering. The kit is a vehicle for musical ideas, not a display of excess.",
      "notes": [
        "Switched from Pearl to a DW custom shop kit in September 2021",
        "DW's signing announcement describes a mix of Collector's Series wood species and shell configurations",
        "Exact shell sizes for the current DW build are not yet independently verified",
        "Earlier Pearl kit used a compact 20\" bass drum for articulation — DW configuration unverified",
        "Setup designed for dynamics and responsiveness, not raw power"
      ],
      "estimatedValue": "Not verified (DW Collector's Series custom shop pricing varies by configuration)"
    },
    "snare": {
      "title": "The DW Collector's Series Snare",
      "brand": "DW",
      "model": "DW Collector's Series Purpleheart",
      "size": "Not verified",
      "shell": "Purpleheart, per DW's Matt Garstka signature drum line",
      "description": "Garstka's Pearl Matt Garstka Signature Snare, played from \"The Madness of Many\" era onward, was retired along with the rest of his Pearl kit when he switched to DW in September 2021. His current primary snare, per DW's Collector's Series signature line, is a Purpleheart-shell model — though DW's custom shop also built him several boutique signature snares (Craviotto, Highwood, Noble & Cooley) that may see use on specific tracks or tours.\n\nExact snare specs, depth, and tuning for the current DW model have not been independently verified beyond the brand and model switch; this article will be updated as more detailed sourcing becomes available. On the earlier Pearl signature model, Matt favored a shallower 14\" x 5\" maple shell for quick response and a wide dynamic range, prioritizing sensitivity for ghost notes while retaining crack for backbeats — the same musical philosophy driving his gear choices today.\n\nFor heads, Matt typically uses Remo Ambassador or Evans G1 batters — medium-weight heads that balance sensitivity with durability. The resonant side usually gets a thin head to maximize snare wire response.",
      "tuningSetting": "Not verified for the current DW model",
      "heads": "Remo Ambassador Coated or Evans G1 Coated (batter), Remo Ambassador Snare Side (resonant)",
      "estimatedValue": "Not verified"
    },
    "cymbals": {
      "title": "Meinl Byzance: Dark Complexity",
      "brand": "Meinl",
      "series": "Meinl Byzance (Multiple lines)",
      "setup": [
        {
          "type": "Hi-Hats",
          "model": "Meinl Byzance 15\" Dual Hi-Hats",
          "position": "Left side",
          "notes": "Complex overtones, dark and washy with clear stick definition"
        },
        {
          "type": "Crash",
          "model": "Meinl Byzance 18\" Extra Dry Thin Crash",
          "position": "Left of hi-hats",
          "notes": "Quick decay, papery attack — perfect for fast accents"
        },
        {
          "type": "Crash",
          "model": "Meinl Byzance 19\" Dual Crash",
          "position": "Over rack toms",
          "notes": "Complex overtones for musical crashes"
        },
        {
          "type": "Crash",
          "model": "Meinl Byzance 20\" Dual Crash",
          "position": "Right of toms",
          "notes": "Larger crash for bigger moments"
        },
        {
          "type": "Ride",
          "model": "Meinl Byzance 22\" Sand Ride",
          "position": "Far right",
          "notes": "Dry articulation with complex wash — signature sound"
        },
        {
          "type": "Stack",
          "model": "Meinl Byzance 18\" Dual Crash + 16\" Trash Crash Stack",
          "position": "Effect position",
          "notes": "Trashy, short accent — modern prog staple"
        },
        {
          "type": "China",
          "model": "Meinl Byzance 18\" Vintage Trash Hat (China)",
          "position": "Far left, high",
          "notes": "Dark, trashy accents without harsh overtones"
        }
      ],
      "description": "Matt Garstka's cymbal setup centers on Meinl's Byzance line — hand-hammered cymbals known for their dark, complex character. Unlike brighter, more aggressive cymbals favored by many metal drummers, Byzance cymbals provide warmth and musicality that suit Matt's dynamic approach.\n\nThe 15\" Dual Hi-Hats are crucial to Matt's sound. The \"Dual\" designation indicates two different finishes (brilliant and raw) combined for complex overtones. These hi-hats respond beautifully to both light stick work and aggressive playing, delivering clear articulation for intricate patterns while maintaining musical warmth.\n\nThe Extra Dry Thin Crash (18\") exemplifies Matt's unconventional choices. Where most metal drummers want explosive, sustaining crashes, Matt prefers quick decay and papery attack. This allows for faster crash patterns without wash buildup — essential for AAL's dense arrangements.\n\nThe 22\" Sand Ride is perhaps Matt's signature cymbal. Its dry, articulate stick definition and complex, controlled wash have made it one of the most popular rides among progressive drummers. The \"sand\" finish (raw/unlathed) contributes to its unique voice.\n\nStacked cymbals provide the trashy, short effects that punctuate modern prog metal. Matt's stacks cut through without overwhelming sustain — accent tools rather than wash generators.\n\nOverall, Matt's cymbal choices prioritize control and musicality over raw volume. Every cymbal serves a specific purpose in his dynamic vocabulary.",
      "estimatedValue": "$3,500-4,500 total (Byzance series setup)"
    },
    "hardware": {
      "title": "Hardware & Vic Firth Sticks",
      "items": [
        {
          "type": "Bass Drum Pedal",
          "brand": "Pearl",
          "model": "Pearl Demon Drive Double Pedal",
          "notes": "Last-verified pedal; brand unconfirmed after the September 2021 DW drum/snare switch",
          "description": "No source covering Garstka's September 2021 DW switch specifies a hardware or pedal brand change, so the Pearl Demon Drive is retained here as his last-verified pedal rather than assumed to have carried over unchanged or switched to DW."
        },
        {
          "type": "Hi-Hat Stand",
          "brand": "Pearl",
          "model": "Pearl Demon Drive Hi-Hat Stand",
          "notes": "Last-verified hardware; brand unconfirmed after the September 2021 DW drum/snare switch"
        },
        {
          "type": "Throne",
          "brand": "Pearl",
          "model": "Pearl Roadster D-3500 Throne",
          "notes": "Last-verified hardware; brand unconfirmed after the September 2021 DW drum/snare switch"
        },
        {
          "type": "Sticks",
          "brand": "Vic Firth",
          "model": "Vic Firth Matt Garstka Signature",
          "notes": "Longer taper for dynamic control, designed for traditional grip",
          "description": "Matt's signature Vic Firth stick features an elongated taper that provides exceptional dynamic control — crucial for his ghost note-heavy style. The stick is designed with traditional grip players in mind, with balance that suits the technique."
        }
      ],
      "heads": {
        "bassKick": "Evans EMAD or Remo Powerstroke 3",
        "toms": "Remo Ambassador Coated or Evans G2 Coated",
        "snare": "Remo Ambassador Coated or Evans G1 Coated"
      }
    },
    "recordingTechniques": {
      "title": "Capturing the Dynamics",
      "content": "Matt Garstka's approach to recording emphasizes dynamics — the very thing that digital production often compresses away. His goal is to capture the full range of his playing, from inaudible ghost notes to thunderous accents.\n\n**Studio Principles:**\n- **Dynamics are sacred**: Matt avoids over-compression that would flatten his dynamic range\n- **Room sound matters**: Natural ambience contributes warmth that samples can't replicate\n- **Minimal triggering**: Unlike many metal drummers, Matt prefers acoustic sounds enhanced rather than replaced\n- **Performance over editing**: His practice regimen means minimal post-production fixes\n\n**Recording \"The Joy of Motion\" (2014):**\nMatt's debut with Animals as Leaders established his recording approach. Working with producer Misha Mansoor (Periphery), he captured performances that showcased both technical precision and musical feel. The drums breathe; you can hear dynamics shift constantly.\n\n**\"The Madness of Many\" (2016) & \"Parrhesia\" (2022):**\nSubsequent albums refined the approach. Matt's sound became even more nuanced, with ghost notes captured at levels most engineers would consider too quiet. The production team understood that these subtleties were essential to his musical identity.\n\n**Live vs. Studio:**\nInterestingly, Matt's live and studio sounds are remarkably similar. He doesn't rely on studio tricks to create his tone — what you hear on record is what you'd hear at a show. This consistency reflects his disciplined practice approach.",
      "studioTips": [
        "Protect dynamics — avoid over-compression",
        "Capture ghost notes; they're essential to the feel",
        "Natural room sound adds warmth digital can't replicate",
        "Practice to performance level before recording",
        "Smaller drums often record better than massive kits"
      ]
    },
    "playingStyle": {
      "title": "The Garstka Approach",
      "content": "Matt Garstka's drumming represents a fusion of jazz sophistication and metal intensity. His approach has influenced countless drummers and expanded what's possible in progressive music.\n\n**Signature Techniques:**\n\n- **Traditional Grip in Metal**: Matt is one of few prominent metal drummers using traditional grip. This influences his touch, dynamics, and positioning — jazz vocabulary applied to heavy music.\n\n- **Ghost Note Vocabulary**: Matt's ghost note patterns are legendary. Where other drummers play spaces, Matt fills them with intricate, barely-audible strokes that add groove and complexity without overwhelming the mix.\n\n- **Metric Modulation**: Matt seamlessly shifts between tempos and feels, making polyrhythmic passages sound natural rather than mathematical. His ability to imply multiple time signatures simultaneously is unmatched in modern metal.\n\n- **Linear Independence**: Each limb operates independently, playing different patterns that combine into complex wholes. This four-way coordination exceeds what most drummers achieve with two hands.\n\n- **Dynamic Extremes**: From whisper-quiet to explosive, Matt uses the full dynamic spectrum. This creates contrast and interest that wall-to-wall intensity can't match.\n\n**Jazz Fusion Roots:**\nMatt cites Vinnie Colaiuta, Steve Gadd, and Tony Williams as influences — jazz drummers rather than metal legends. This background informs his melodic approach to drumming, where drums contribute to the music rather than just providing a foundation.\n\n**The AAL Transformation:**\nWhen Matt joined Animals as Leaders, he transformed the band's rhythmic approach. Earlier albums featured excellent drumming; Matt's albums feature drumming as a compositional voice. His parts are melodies in their own right.",
      "signatureSongs": [
        {
          "song": "Physical Education",
          "album": "The Joy of Motion",
          "note": "Ghost note masterclass"
        },
        {
          "song": "Tooth and Claw",
          "album": "The Joy of Motion",
          "note": "Polyrhythmic complexity"
        },
        {
          "song": "The Brain Dance",
          "album": "The Madness of Many",
          "note": "Technical showcase"
        },
        {
          "song": "Arithmophobia",
          "album": "The Madness of Many",
          "note": "Metric modulation demonstration"
        },
        {
          "song": "Monomyth",
          "album": "Parrhesia",
          "note": "Dynamic range and feel"
        },
        {
          "song": "Gestaltzerfall",
          "album": "Parrhesia",
          "note": "Modern Matt at his peak"
        }
      ]
    },
    "gearTimeline": [
      {
        "era": "Pre-AAL",
        "years": "2007-2012",
        "albums": [
          "Various session work"
        ],
        "description": "Berklee years and early professional work, developing the jazz-fusion vocabulary.",
        "gear": {
          "drums": "Various kits during Berklee",
          "snare": "Ludwig Supraphonic (jazz standard)",
          "cymbals": "Zildjian K series",
          "hardware": "Various"
        },
        "notes": "Matt's formative years at Berklee established his jazz foundation before entering the prog metal world."
      },
      {
        "era": "Early AAL",
        "years": "2012-2016",
        "albums": [
          "The Joy of Motion"
        ],
        "description": "Joined Animals as Leaders, established signature sound.",
        "gear": {
          "drums": "Pearl Masterworks Maple",
          "snare": "Pearl Reference (pre-signature)",
          "cymbals": "Meinl Byzance series",
          "hardware": "Pearl Demon Drive"
        },
        "notes": "\"The Joy of Motion\" introduced the world to Matt's unique approach — jazz vocabulary in a djent context."
      },
      {
        "era": "Peak Era",
        "years": "2016-2022",
        "albums": [
          "The Madness of Many",
          "Parrhesia"
        ],
        "description": "Signature products developed, educational presence expanded.",
        "gear": {
          "drums": "Pearl Masterworks Maple (refined setup)",
          "snare": "Pearl Matt Garstka Signature",
          "cymbals": "Meinl Byzance (expanded setup)",
          "hardware": "Pearl Demon Drive, Vic Firth Signature Sticks"
        },
        "notes": "Matt's signature snare and sticks released. Drumeo and clinic work made him one of the most-watched drummers online."
      },
      {
        "era": "Current",
        "years": "2023-Present",
        "albums": [
          "Touring Parrhesia, new music TBA"
        ],
        "description": "Continued touring, education, and evolution on a new DW kit.",
        "gear": {
          "drums": "DW Collector's Series (custom shop, switched from Pearl in September 2021)",
          "snare": "DW Collector's Series Purpleheart",
          "cymbals": "Meinl Byzance (various configurations per tour)",
          "hardware": "Pearl Demon Drive (last-verified, brand unconfirmed post-switch), Vic Firth Signature Sticks"
        },
        "notes": "Matt continues to refine his approach while maintaining the core philosophy of dynamics and musicality."
      }
    ],
    "videos": [
      {
        "youtubeId": "bt-RoSzsEKA",
        "title": "Matt Garstka Drumeo Lesson",
        "description": "Comprehensive technique breakdown"
      }
    ],
    "quotes": [
      {
        "text": "Dynamics are everything. If you play at one volume, you're not really playing music — you're just making noise.",
        "source": "Drumeo Interview",
        "year": 2018
      },
      {
        "text": "I don't think about playing 'metal' or 'jazz.' I think about what the music needs at that moment.",
        "source": "Modern Drummer",
        "year": 2020
      },
      {
        "text": "Ghost notes aren't decoration. They're the groove. Take them away and the whole thing falls apart.",
        "source": "Drummer's Resource Podcast",
        "year": 2019
      }
    ],
    "gearStillAvailable": {
      "title": "Matt's Gear You Can Buy Today",
      "items": [
        {
          "item": "Pearl Matt Garstka Signature Snare",
          "available": true,
          "priceRange": "$400-500",
          "notes": "Available at most Pearl dealers — his signature model from his 2016-2021 Pearl era, before his September 2021 switch to DW"
        },
        {
          "item": "Vic Firth Matt Garstka Signature Sticks",
          "available": true,
          "priceRange": "$12-15 per pair",
          "notes": "Widely available at music retailers"
        },
        {
          "item": "Meinl Byzance Cymbals",
          "available": true,
          "priceRange": "$200-500 per cymbal",
          "notes": "Full Byzance line available — Sand Ride and Dual series are Matt's choices"
        },
        {
          "item": "Pearl Demon Drive Pedals",
          "available": true,
          "priceRange": "$400-700",
          "notes": "Last-verified pedal Matt used; brand unconfirmed after his September 2021 switch to DW drums"
        },
        {
          "item": "Pearl Masterworks Drums",
          "available": true,
          "priceRange": "$4,000-7,000",
          "notes": "Custom configurations available through Pearl dealers — Matt's kit before his September 2021 switch to DW; DW Collector's Series custom shop pricing is not verified"
        }
      ]
    },
    "buyingGuide": {
      "title": "Building a Garstka-Inspired Setup",
      "intro": "Capturing Matt Garstka's sound requires focusing on responsiveness and dynamics over raw power. Here's how to approach it at different budgets.",
      "sections": [
        {
          "tier": "Budget Build (~$2,000)",
          "items": [
            "Pearl Export series (20\" bass, 10/12/14 toms) — $800",
            "Meinl Classics Custom Dark series cymbals — $600",
            "Pearl P-930 Demonator pedal — $150",
            "Remo Ambassador heads all around — $100",
            "Medium-weight sticks with long taper — $15"
          ],
          "notes": "Focus on smaller drums and responsive cymbals. The Classics Custom Dark line offers Byzance-like darkness at a lower price."
        },
        {
          "tier": "Mid-Range Build (~$4,500)",
          "items": [
            "Pearl Session Studio Select (20\" bass) — $2,000",
            "Meinl Byzance Traditional/Extra Dry mix — $1,500",
            "Pearl Demon Drive double pedal — $500",
            "Pearl Matt Garstka Signature Snare — $450",
            "Vic Firth Matt Garstka Signature Sticks — $15"
          ],
          "notes": "The Session Studio delivers Masterworks-quality tones. Adding Matt's signature snare and sticks gets you closer to his actual sound."
        },
        {
          "tier": "Pro Build (~$8,000+)",
          "items": [
            "Pearl Masterworks Maple (custom sizes) — $4,500",
            "Full Meinl Byzance setup (Dual hi-hats, Extra Dry crashes, Sand Ride) — $2,500",
            "Pearl Demon Drive pedals + hi-hat stand — $900",
            "Pearl Matt Garstka Signature Snare — $450",
            "Vic Firth Matt Garstka Signature Sticks — $15"
          ],
          "notes": "This replicates Matt's Pearl-era signature setup (2016-2021). His current kit is a DW Collector's Series custom shop build (switched September 2021); DW custom shop pricing is not verified. Remember: the gear is just the start — his sound comes from years of disciplined practice."
        }
      ]
    },
    "education": {
      "title": "Learn From Matt",
      "content": "Matt Garstka is one of the most prolific drum educators of his generation. His teaching emphasizes understanding rather than imitation — learn the concepts, then apply them your way.\n\n**Drumeo Content:**\nMatt's Drumeo appearances are legendary. His lessons on ghost notes, metric modulation, and dynamic control have millions of views. The platform offers structured courses featuring his techniques.\n\n**Clinic Tours:**\nMatt tours internationally doing drum clinics. These combine performance, explanation, and Q&A — opportunities to see his approach up close and ask questions.\n\n**Transcriptions:**\nUnusually, Matt provides detailed transcriptions of his own parts. These are invaluable for understanding his thought process — not just what he plays, but why.\n\n**Social Media:**\nInstagram and YouTube feature regular content: practice clips, gear demos, and technique tips. Following Matt's channels provides ongoing education.",
      "resources": [
        {
          "name": "Drumeo Matt Garstka Courses",
          "type": "Video lessons",
          "url": "https://www.drumeo.com"
        },
        {
          "name": "Matt Garstka Official YouTube",
          "type": "Free content",
          "url": "https://www.youtube.com/@mattgarstka"
        },
        {
          "name": "Vic Firth Education",
          "type": "Technique videos",
          "url": "https://vicfirth.com"
        },
        {
          "name": "Meinl Cymbals",
          "type": "Gear demos",
          "url": "https://meinlcymbals.com"
        }
      ]
    },
    "relatedAlbums": [
      "the-joy-of-motion-drum-setup",
      "the-madness-of-many-drum-setup",
      "parrhesia-drum-setup",
      "obzen-drum-setup",
      "fear-inoculum-drum-setup"
    ],
    "relatedDrummers": [
      5,
      14,
      13
    ],
    "relatedArticles": [
      "best-cymbals-for-prog-metal",
      "traditional-grip-in-metal"
    ],
    "internalLinks": [
      {
        "text": "Matt Garstka full biography",
        "url": "/drummers/matt-garstka"
      },
      {
        "text": "Animals as Leaders band profile",
        "url": "/bands/animals-as-leaders"
      },
      {
        "text": "DW Drums brand guide",
        "url": "/brands/dw"
      },
      {
        "text": "Meinl Cymbals brand guide",
        "url": "/brands/meinl"
      },
      {
        "text": "Tomas Haake kit article",
        "url": "/articles/whats-in-tomas-haakes-kit"
      },
      {
        "text": "Danny Carey kit article",
        "url": "/articles/whats-in-danny-careys-kit"
      }
    ],
    "faq": [
      {
        "question": "What drum kit does Matt Garstka use?",
        "answer": "Matt Garstka plays a DW Collector's Series custom shop kit, mixing Collector's Series wood species and shell configurations per DW's official signing announcement. He switched to DW in September 2021, moving on from the Pearl Masterworks Maple kit he'd played since \"The Joy of Motion\" (2014). Exact shell sizes for the current DW build have not been independently verified."
      },
      {
        "question": "What cymbals does Matt Garstka use?",
        "answer": "Matt Garstka uses Meinl Byzance cymbals across multiple lines. His setup includes Byzance 15 inch Dual Hi-Hats, Byzance crashes in 18 and 20 inch sizes, a Byzance 22 inch Sand Ride, a Byzance 18 inch Vintage Trash Hat used as a China, and various stack effects. The Byzance line's handcrafted Turkish feel suits his blend of jazz-fusion sensitivity and progressive metal attack."
      },
      {
        "question": "How much does Matt Garstka's drum kit cost?",
        "answer": "Matt Garstka's current DW Collector's Series custom shop kit and DW Collector's Series Purpleheart snare are custom shop orders, so exact pricing has not been independently verified. His prior Pearl Masterworks Maple shell pack (played through 2021) ran approximately $4,000 to $6,000 new, with his Pearl Matt Garstka Signature Snare adding around $400 to $500. His Meinl Byzance cymbal setup can easily exceed $2,500 to $4,000 when factoring in multiple specialty pieces."
      }
    ],
    "conclusion": {
      "title": "The Future of Progressive Drumming",
      "content": "Matt Garstka has redefined what's possible in progressive metal drumming. By bringing jazz sophistication to heavy music — traditional grip, ghost note vocabulary, dynamic extremes — he's created an approach that countless drummers now study and emulate.\n\nHis gear choices reflect his philosophy: quality over quantity, responsiveness over power, musicality over aggression. A relatively compact DW Collector's Series kit (switched from Pearl in September 2021), dark Meinl Byzance cymbals, and signature products designed for dynamic control rather than brute force.\n\n**What Makes Matt Special:**\nIt's not the speed (though he's plenty fast). It's not the complexity (though his patterns are intricate). It's the musicality — the way every stroke serves the composition, the way dynamics create emotion, the way jazz vocabulary enriches metal intensity.\n\n**For Drummers Seeking This Sound:**\nStart with concepts, not gear. Practice ghost notes until they're automatic. Work on dynamic control before worrying about speed. Study jazz drummers alongside metal ones. Matt's sound comes from his approach, not his equipment.\n\n**The Legacy:**\nAnimals as Leaders' rhythmic voice is inseparable from Matt Garstka. He didn't just join a band — he transformed its sonic identity. And through his educational work, he's transformed how a generation thinks about progressive drumming.\n\nWhether you're building a Garstka-inspired setup or just seeking to understand modern prog drumming, his philosophy is clear: dynamics, musicality, and creativity matter more than raw power. In a genre often defined by extremity, that's a revolutionary idea.\n\nFor the era-by-era breakdown of how this kit evolved across each Animals as Leaders record, see [The Joy of Motion drum setup](/articles/the-joy-of-motion-drum-setup), [The Madness of Many drum setup](/articles/the-madness-of-many-drum-setup), and [Parrhesia drum setup](/articles/parrhesia-drum-setup).\n\n🥁 *Matt Garstka — proving that the most sophisticated approach can also be the heaviest.* 🥁"
    }
  },
  "the-joy-of-motion-drum-setup": {
    "slug": "the-joy-of-motion-drum-setup",
    "albumTitle": "The Joy of Motion",
    "artist": "Animals as Leaders",
    "drummer": "Matt Garstka",
    "drummerFullName": "Matt Garstka",
    "drummerId": 53,
    "relatedDrummerSlug": "matt-garstka",
    "year": 2014,
    "genre": "Progressive Metal / Djent",
    "label": "Sumerian Records",
    "producer": "Misha Mansoor",
    "isAlbumArticle": true,
    "articleType": "album-drum-setup",
    "datePublished": "2026-07-08",
    "dateModified": "2026-07-08",
    "author": "MetalForge Editorial",
    "title": "The Joy of Motion Drum Setup: Matt Garstka's Debut With Animals as Leaders",
    "description": "Matt Garstka's drum gear on Animals as Leaders' The Joy of Motion (2014) — the Pearl Masterworks Maple kit, Pearl Reference snare, and Meinl Byzance cymbals behind his career-defining debut with the band.",
    "seoKeywords": [
      "the joy of motion drum setup",
      "matt garstka the joy of motion",
      "animals as leaders 2014 drums",
      "matt garstka pearl masterworks",
      "matt garstka debut album drums",
      "the joy of motion drum kit",
      "animals as leaders drummer gear",
      "matt garstka sumerian records"
    ],
    "ogImage": "/images/drummers/matt-garstka.webp",
    "intro": {
      "title": "The Album That Introduced Garstka to the World",
      "content": "Released March 25, 2014 on Sumerian Records, \"The Joy of Motion\" is Matt Garstka's first studio album with Animals as Leaders, recorded roughly two years after he replaced founding drummer Navene Koperweis in 2012. Where the band's first two records established their instrumental djent identity, \"The Joy of Motion\" is the moment that identity absorbed Garstka's Berklee-trained, jazz-fusion vocabulary — a shift audible from the opening bars of \"Kascade.\"\n\nWorking with producer Misha Mansoor of Periphery, Garstka tracked a performance that prioritized dynamic range over sheer density. Tracks like \"Physical Education\" and \"Tooth and Claw\" showcase the ghost-note-heavy, traditional-grip approach that would come to define his career, while \"Crescent\" and \"Air Chrysalis\" demonstrate the metric modulation and polyrhythmic layering he'd been developing since his session years.\n\nGarstka's gear on this record reflects a drummer still calibrating his setup for the demands of Animals as Leaders' material: a Pearl Masterworks Maple kit sized for articulation rather than raw power, a pre-signature Pearl Reference snare, and an early iteration of the Meinl Byzance cymbal stack that would later expand into his full touring rig. This article breaks down the gear behind Garstka's breakout performance — see [What's In Matt Garstka's Kit](/articles/whats-in-matt-garstkas-kit) for his complete career-spanning equipment guide, and [Matt Garstka's full drummer profile](/drummer/matt-garstka) for his biography.",
      "keyPoints": [
        "Released March 25, 2014 on Sumerian Records — Garstka's first album with Animals as Leaders",
        "Produced with Misha Mansoor (Periphery), emphasizing dynamic range over compression",
        "Pearl Masterworks Maple kit — compact four-piece configuration for articulation",
        "Pre-signature Pearl Reference snare — precursor to Garstka's later signature model",
        "\"Physical Education\" and \"Tooth and Claw\" remain the definitive ghost-note and polyrhythm showcases from this era"
      ]
    },
    "drumKit": {
      "title": "The Early Pearl Masterworks Setup",
      "brand": "Pearl",
      "model": "Pearl Masterworks Maple",
      "finish": "Natural Maple",
      "config": {
        "bassdrums": [
          "20\" x 16\" Bass Drum"
        ],
        "toms": [
          "10\" x 7\" Rack Tom",
          "12\" x 8\" Rack Tom"
        ],
        "floorToms": [
          "14\" x 14\" Floor Tom"
        ],
        "shells": "Maple shells with MasterCast hoops"
      },
      "description": "For \"The Joy of Motion,\" Garstka brought the same compact philosophy to the studio that would define his entire career: a four-piece Pearl Masterworks Maple kit built around a 20\" bass drum rather than the 22\" or 24\" shells common in metal. The smaller bass drum gave the intricate kick patterns on tracks like \"Kascade\" the quick attack and clear articulation needed at the tempos Animals as Leaders' material demands.\n\nThe 10\" and 12\" rack toms and 14\" floor tom kept the kit's footprint tight, forcing musical decisions over pure size. Masterworks maple shells provided the warmth that let Garstka's dynamic playing — from near-silent ghost notes to full accents — translate clearly onto tape, while Pearl's MasterCast hoops added the focused attack that cuts through the album's dense, layered guitar production.\n\nThis was Garstka's first time recording a full album with Animals as Leaders, and the kit choice reflects a deliberate statement: rather than matching the band's technical density with a bigger rig, he used a smaller one to force clarity and space into every part.",
      "notes": [
        "20\" bass drum — smaller than typical metal setups, chosen for articulation at speed",
        "Compact four-piece configuration matching Garstka's minimalist philosophy",
        "Masterworks maple shells for warmth under dense production",
        "MasterCast hoops for focused attack in the mix",
        "First full album recorded with Animals as Leaders after joining in 2012"
      ],
      "estimatedValue": "$4,000-6,000 (Masterworks Maple configuration)"
    },
    "snare": {
      "title": "The Pre-Signature Pearl Reference Snare",
      "brand": "Pearl",
      "model": "Pearl Reference",
      "size": "14\" x 5\"",
      "shell": "Maple",
      "description": "Before Pearl developed his signature model, Garstka recorded \"The Joy of Motion\" on a Pearl Reference maple snare — the drum that established the sonic template his later signature snare would refine. At 14\" x 5\", it's shallower than many metal snares, prioritizing quick response and dynamic range over sheer volume.\n\nThat dynamic range is essential to how \"The Joy of Motion\" reads as a recording. Songs like \"Physical Education\" alternate between barely-audible ghost notes and full-force backbeats within a single bar, and the Reference snare's maple shell speaks clearly across that entire spectrum without the harshness that can plague brighter metal snares.\n\nGarstka tuned the drum at medium tension, balancing sensitivity for ghost note work with enough crack to cut through Tosin Abasi and Javier Reyes's layered eight-string guitar tracking. This snare tone — warm, articulate, dynamically responsive — became the acoustic signature that carried through to his later Pearl Matt Garstka Signature model.",
      "tuningSetting": "Medium tension for ghost note sensitivity and backbeat crack",
      "heads": "Remo Ambassador Coated (batter), Remo Ambassador Snare Side (resonant)",
      "estimatedValue": "$350-450 (Pearl Reference series)"
    },
    "cymbals": {
      "title": "Early Byzance: Establishing the Dark, Complex Voice",
      "brand": "Meinl",
      "series": "Meinl Byzance",
      "setup": [
        {
          "type": "Hi-Hats",
          "model": "Meinl Byzance 15\" Dual Hi-Hats",
          "position": "Left side",
          "notes": "Complex overtones with clear stick definition for intricate patterns"
        },
        {
          "type": "Crash",
          "model": "Meinl Byzance 18\" Extra Dry Thin Crash",
          "position": "Left of hi-hats",
          "notes": "Quick decay for fast accents without wash buildup"
        },
        {
          "type": "Crash",
          "model": "Meinl Byzance 19\" Dual Crash",
          "position": "Over rack toms",
          "notes": "Complex, musical crash tone"
        },
        {
          "type": "Ride",
          "model": "Meinl Byzance 22\" Sand Ride",
          "position": "Far right",
          "notes": "Dry articulation that became Garstka's signature ride voice"
        },
        {
          "type": "China",
          "model": "Meinl Byzance 18\" Vintage Trash Hat (China)",
          "position": "Far left, high",
          "notes": "Dark, trashy accents for effect passages"
        }
      ],
      "description": "\"The Joy of Motion\" documents an earlier, slightly leaner version of the Meinl Byzance setup Garstka would later expand for \"The Madness of Many\" and \"Parrhesia.\" The core voices are already in place — the Dual Hi-Hats, the Extra Dry Thin Crash, and most notably the 22\" Sand Ride, which became one of the most recognizable cymbal choices in modern progressive metal.\n\nThe Sand Ride's dry, articulate stick definition combined with a controlled wash gave Garstka a ride sound distinct from the bright, aggressive rides common in metal at the time — a choice that suited the jazz vocabulary he was bringing into the band. The Extra Dry Thin Crash's fast decay let him crash through fills on \"Air Chrysalis\" without the cymbal's sustain bleeding into subsequent phrases, a critical consideration on an album built from tightly layered polyrhythms.\n\nCompared to his later, more elaborate stacks and multiple crash configurations, this early Byzance setup is comparatively streamlined — the sound of a drummer establishing his core cymbal voice before expanding it.",
      "estimatedValue": "$2,500-3,000 (early Byzance configuration)"
    },
    "hardware": {
      "title": "Pearl Demon Drive and Vic Firth Sticks",
      "items": [
        {
          "type": "Bass Drum Pedal",
          "brand": "Pearl",
          "model": "Pearl Demon Drive Double Pedal",
          "notes": "Direct drive mechanism for instant response",
          "description": "Garstka's direct-drive Demon Drive pedal gave him the immediate response needed for the intricate kick patterns on tracks like \"Kascade\" — any chain-drive lag would have disrupted the precise polyrhythmic relationships between kick and hands."
        },
        {
          "type": "Hi-Hat Stand",
          "brand": "Pearl",
          "model": "Pearl Demon Drive Hi-Hat Stand",
          "notes": "Matching direct-drive feel for consistent hi-hat response"
        },
        {
          "type": "Sticks",
          "brand": "Vic Firth",
          "model": "Vic Firth American Classic 5A",
          "notes": "Standard model used before his signature stick was developed"
        }
      ],
      "heads": {
        "bassKick": "Remo Powerstroke 3",
        "toms": "Remo Ambassador Coated",
        "snare": "Remo Ambassador Coated"
      }
    },
    "playingStyle": {
      "title": "Introducing the Garstka Vocabulary",
      "content": "\"The Joy of Motion\" is the recording where Matt Garstka's playing vocabulary first reached a wide audience, and its defining qualities are all present at full strength: traditional grip, dense ghost-note phrasing, and metric modulation applied to djent's angular riffing.\n\nOn \"Physical Education,\" Garstka's ghost notes fill the spaces between Tosin Abasi and Javier Reyes's eight-string riffing with a level of detail rarely heard in the genre — patterns that owe more to Vinnie Colaiuta and Steve Gadd than to metal's blast-beat lineage. \"Tooth and Claw\" pushes the polyrhythmic layering further, with Garstka implying multiple time feels simultaneously without ever losing the song's underlying pulse.\n\nThe traditional grip he uses throughout the album — unusual among prog-metal and djent drummers — shapes his touch on the snare and hi-hat, giving his ghost notes a controlled, jazz-inflected feel that became his signature. His rudiment-derived linear fills, discussed in detail on MetalForge's [rudiments technique guide](/technique/rudiments), are already fully formed here, splitting single-stroke and paradiddle-family patterns between hands and feet.\n\nFor drummers studying this era of Garstka's playing, \"The Joy of Motion\" is the essential starting point — the album where his identity as a drummer first became inseparable from Animals as Leaders' sound. See [Matt Garstka's full drummer profile](/drummer/matt-garstka) for his complete career arc, and [The Madness of Many drum setup](/articles/the-madness-of-many-drum-setup) for how his gear and technique evolved on the follow-up record.",
      "signatureSongs": [
        {
          "song": "Physical Education",
          "album": "The Joy of Motion",
          "note": "Ghost note masterclass"
        },
        {
          "song": "Tooth and Claw",
          "album": "The Joy of Motion",
          "note": "Polyrhythmic complexity"
        },
        {
          "song": "Kascade",
          "album": "The Joy of Motion",
          "note": "Precise, articulate kick patterns"
        }
      ]
    },
    "faq": [
      {
        "question": "What drum kit did Matt Garstka use on The Joy of Motion?",
        "answer": "Matt Garstka recorded The Joy of Motion (2014) on a Pearl Masterworks Maple kit configured with a 20x16 inch bass drum, 10x7 and 12x8 inch rack toms, and a 14x14 inch floor tom. The compact four-piece configuration, built from maple shells with Pearl's MasterCast hoops, prioritized quick attack and articulation over raw size — a deliberate choice given the album's dense, technical arrangements."
      },
      {
        "question": "What snare did Matt Garstka use on The Joy of Motion?",
        "answer": "Garstka used a Pearl Reference 14x5 inch maple snare on The Joy of Motion, recorded before Pearl developed his later signature model. Tuned to medium tension, it balanced sensitivity for his extensive ghost-note vocabulary with enough crack to cut through Animals as Leaders' layered eight-string guitar tracking."
      },
      {
        "question": "Who produced The Joy of Motion?",
        "answer": "The Joy of Motion was produced by Misha Mansoor of Periphery. The production emphasized dynamic range over heavy compression, allowing Garstka's ghost notes and dynamic contrasts — central to his playing style — to remain audible in the final mix rather than being flattened."
      },
      {
        "question": "What cymbals did Matt Garstka use on The Joy of Motion?",
        "answer": "Garstka used an early configuration of Meinl Byzance cymbals on The Joy of Motion, including 15 inch Dual Hi-Hats, an 18 inch Extra Dry Thin Crash, a 19 inch Dual Crash, a 22 inch Sand Ride, and an 18 inch Vintage Trash Hat used as a China. The Sand Ride in particular became one of his signature cymbal choices, carried forward across his later Animals as Leaders albums."
      },
      {
        "question": "Was The Joy of Motion Matt Garstka's first album with Animals as Leaders?",
        "answer": "Yes. Garstka joined Animals as Leaders in 2012, replacing founding drummer Navene Koperweis, and The Joy of Motion (released March 25, 2014 on Sumerian Records) was his first full studio album with the band. It established the jazz-fusion-influenced, traditional-grip playing style that would define his career."
      }
    ],
    "videos": [],
    "relatedAlbums": [
      "whats-in-matt-garstkas-kit",
      "the-madness-of-many-drum-setup",
      "parrhesia-drum-setup"
    ],
    "relatedDrummers": [
      5,
      14,
      13
    ],
    "relatedArticles": [
      "best-cymbals-for-prog-metal",
      "traditional-grip-in-metal"
    ],
    "conclusion": {
      "title": "The Album That Started It All",
      "content": "\"The Joy of Motion\" remains the foundational document of Matt Garstka's career — the recording where his Berklee-trained jazz vocabulary first collided with Animals as Leaders' progressive djent, producing a sound that redefined what technical metal drumming could be. His Pearl Masterworks kit, pre-signature Reference snare, and early Byzance cymbal stack may look modest next to his later, more elaborate rigs, but the musical DNA — dynamics, ghost notes, traditional grip, metric modulation — is already fully realized.\n\nFor drummers tracing how Garstka's gear and playing evolved, this album is the essential starting point before moving to [The Madness of Many drum setup](/articles/the-madness-of-many-drum-setup) and [Parrhesia drum setup](/articles/parrhesia-drum-setup). For his complete equipment history across every era, see [What's In Matt Garstka's Kit](/articles/whats-in-matt-garstkas-kit), and for his full biography, visit [Matt Garstka's drummer profile](/drummer/matt-garstka)."
    }
  },
  "the-madness-of-many-drum-setup": {
    "slug": "the-madness-of-many-drum-setup",
    "albumTitle": "The Madness of Many",
    "artist": "Animals as Leaders",
    "drummer": "Matt Garstka",
    "drummerFullName": "Matt Garstka",
    "drummerId": 53,
    "relatedDrummerSlug": "matt-garstka",
    "year": 2016,
    "genre": "Progressive Metal / Djent",
    "label": "Sumerian Records",
    "isAlbumArticle": true,
    "articleType": "album-drum-setup",
    "datePublished": "2026-07-08",
    "dateModified": "2026-07-08",
    "author": "MetalForge Editorial",
    "title": "The Madness of Many Drum Setup: Matt Garstka's Signature Gear Era Begins",
    "description": "Matt Garstka's drum gear on Animals as Leaders' The Madness of Many (2016) — the refined Pearl Masterworks setup, expanded Meinl Byzance cymbal stack, and the arrival of his signature Pearl snare and Vic Firth sticks.",
    "seoKeywords": [
      "the madness of many drum setup",
      "matt garstka the madness of many",
      "animals as leaders 2016 drums",
      "matt garstka signature snare",
      "the madness of many drum kit",
      "matt garstka byzance cymbals",
      "animals as leaders drummer gear",
      "the brain dance drums"
    ],
    "ogImage": "/images/drummers/matt-garstka.webp",
    "intro": {
      "title": "Refinement and the Arrival of Signature Gear",
      "content": "Released November 11, 2016 on Sumerian Records, \"The Madness of Many\" is Matt Garstka's second album with Animals as Leaders and the record that marks his transition from rising talent to established genre voice. Where \"The Joy of Motion\" introduced his vocabulary, \"The Madness of Many\" refines it — the metric modulation on \"Arithmophobia\" and the technical density of \"The Brain Dance\" push his four-way independence further than anything on the debut.\n\nThis era also coincides with the beginning of Garstka's signature product development. Pearl's Matt Garstka Signature Snare and Vic Firth's signature stick model both trace back to this stretch of his career, as his profile grew through Drumeo lessons and international clinics running in parallel with the band's touring cycle. His gear setup for this album reflects that transition — a more refined version of the Pearl Masterworks kit, an expanded Meinl Byzance cymbal stack, and the earliest use of the drum and stick designs that would carry his name.\n\nThis article covers the gear behind \"The Madness of Many\" specifically. For the setup he used two years earlier, see [The Joy of Motion drum setup](/articles/the-joy-of-motion-drum-setup); for his complete career-spanning gear history, see [What's In Matt Garstka's Kit](/articles/whats-in-matt-garstkas-kit) and [Matt Garstka's drummer profile](/drummer/matt-garstka).",
      "keyPoints": [
        "Released November 11, 2016 on Sumerian Records — Garstka's second Animals as Leaders album",
        "Marks the beginning of Garstka's Pearl and Vic Firth signature product era",
        "Refined Pearl Masterworks Maple setup with an expanded Meinl Byzance cymbal stack",
        "\"The Brain Dance\" and \"Arithmophobia\" showcase his most demanding metric modulation to date",
        "Coincides with Garstka's rise as a Drumeo educator alongside his touring schedule"
      ]
    },
    "drumKit": {
      "title": "The Refined Masterworks Setup",
      "brand": "Pearl",
      "model": "Pearl Masterworks Maple",
      "finish": "Natural Maple",
      "config": {
        "bassdrums": [
          "20\" x 16\" Bass Drum"
        ],
        "toms": [
          "10\" x 7\" Rack Tom",
          "12\" x 8\" Rack Tom"
        ],
        "floorToms": [
          "14\" x 14\" Floor Tom"
        ],
        "shells": "Maple shells with MasterCast hoops"
      },
      "description": "Garstka carried the same core Pearl Masterworks Maple configuration into \"The Madness of Many\" that had defined \"The Joy of Motion\" — the 20\" bass drum, 10\" and 12\" rack toms, and 14\" floor tom remained his template for articulation-focused metal drumming. What changed was how precisely dialed-in the setup had become after two years of touring the previous album.\n\nThe technical demands of tracks like \"The Brain Dance\" — among the most rhythmically dense material Animals as Leaders had written to that point — put a premium on a kit that could keep every stroke distinct even during the fastest linear passages. The Masterworks maple shells and MasterCast hoops continued to deliver that clarity, now shaped by a drummer with two additional years of road-tested familiarity with exactly how his kit responded under pressure.\n\nThis is the same core drum configuration documented across Garstka's \"Peak Era\" in his general gear timeline — the setup that stayed constant even as his snare and stick choices evolved into signature territory.",
      "notes": [
        "Same 20\" bass drum / 10\", 12\" rack tom / 14\" floor tom configuration as The Joy of Motion",
        "Two additional years of touring refinement between albums",
        "Built for the technical density of tracks like \"The Brain Dance\"",
        "Masterworks maple shells and MasterCast hoops for consistent articulation",
        "Part of the broader \"Peak Era\" (2016-2022) documented in Garstka's full gear timeline"
      ],
      "estimatedValue": "$4,000-6,000 (Masterworks Maple configuration)"
    },
    "snare": {
      "title": "The New Pearl Matt Garstka Signature Snare",
      "brand": "Pearl",
      "model": "Pearl Matt Garstka Signature Snare",
      "size": "14\" x 5\"",
      "shell": "Maple with unique bearing edge",
      "description": "\"The Madness of Many\" falls within the era Pearl developed Garstka's signature snare — a 14\" x 5\" maple drum built around a custom bearing edge designed specifically for his dynamic, ghost-note-heavy playing. Where the Pearl Reference he used on \"The Joy of Motion\" was an existing production model, this signature drum was engineered around his specific technique.\n\nThe custom bearing edge gives the drum exceptional sensitivity at low volumes without sacrificing the crack needed for full-force backbeats — a dual character essential on a track like \"Arithmophobia,\" where Garstka's playing shifts between whisper-quiet ghost note clusters and explosive accents within the same phrase. The maple shell keeps that entire dynamic range warm rather than harsh, even under the aggressive compression common in modern metal mixing.\n\nThis snare became closely associated with Garstka's identity as a drummer — the instrument most directly shaped by his own input, and the clearest gear signal that he'd moved from simply endorsing Pearl products to co-designing them.",
      "tuningSetting": "Medium tension for balance of sensitivity and crack",
      "heads": "Remo Ambassador Coated or Evans G1 Coated (batter), Remo Ambassador Snare Side (resonant)",
      "estimatedValue": "$400-500 (signature model)"
    },
    "cymbals": {
      "title": "The Expanded Byzance Stack",
      "brand": "Meinl",
      "series": "Meinl Byzance (Multiple lines)",
      "setup": [
        {
          "type": "Hi-Hats",
          "model": "Meinl Byzance 15\" Dual Hi-Hats",
          "position": "Left side",
          "notes": "Complex overtones, dark and washy with clear stick definition"
        },
        {
          "type": "Crash",
          "model": "Meinl Byzance 18\" Extra Dry Thin Crash",
          "position": "Left of hi-hats",
          "notes": "Quick decay, papery attack for fast accents"
        },
        {
          "type": "Crash",
          "model": "Meinl Byzance 19\" Dual Crash",
          "position": "Over rack toms",
          "notes": "Complex overtones for musical crashes"
        },
        {
          "type": "Crash",
          "model": "Meinl Byzance 20\" Dual Crash",
          "position": "Right of toms",
          "notes": "Added for this era — larger crash for bigger dynamic moments"
        },
        {
          "type": "Ride",
          "model": "Meinl Byzance 22\" Sand Ride",
          "position": "Far right",
          "notes": "Dry articulation with complex wash — Garstka's signature ride"
        },
        {
          "type": "Stack",
          "model": "Meinl Byzance 18\" Dual Crash + 16\" Trash Crash Stack",
          "position": "Effect position",
          "notes": "New trashy accent stack for this era's material"
        },
        {
          "type": "China",
          "model": "Meinl Byzance 18\" Vintage Trash Hat (China)",
          "position": "Far left, high",
          "notes": "Dark, trashy accents without harsh overtones"
        }
      ],
      "description": "Garstka's Byzance setup expanded significantly for \"The Madness of Many,\" growing from the leaner configuration on \"The Joy of Motion\" into the full stack that would become his standard touring rig. The additions — a 20\" Dual Crash and an 18\" Dual Crash / 16\" Trash Crash effect stack — gave him a wider dynamic and textural palette for the album's denser arrangements.\n\nThe 20\" Dual Crash provided a bigger, more sustained crash option for the album's most dramatic moments, complementing the quicker-decaying 18\" Extra Dry Thin Crash he'd used since \"The Joy of Motion.\" The new effect stack added the short, trashy accent sound increasingly common in modern progressive metal production — a texture that punctuates transitions on tracks like \"Ectogenesis\" without the extended wash a full crash or china would produce.\n\nThe 22\" Sand Ride remained the constant centerpiece across both albums, its dry articulation and complex wash cementing its status as one of the most recognized cymbal voices in progressive drumming.",
      "estimatedValue": "$3,500-4,500 total (expanded Byzance setup)"
    },
    "hardware": {
      "title": "Pearl Demon Drive and the New Vic Firth Signature Stick",
      "items": [
        {
          "type": "Bass Drum Pedal",
          "brand": "Pearl",
          "model": "Pearl Demon Drive Double Pedal",
          "notes": "Direct drive for precision and speed",
          "description": "Garstka continued using the Pearl Demon Drive's direct-drive mechanism on The Madness of Many, its instant response remaining essential for the intricate kick patterns underlying tracks like \"Arithmophobia.\""
        },
        {
          "type": "Hi-Hat Stand",
          "brand": "Pearl",
          "model": "Pearl Demon Drive Hi-Hat Stand",
          "notes": "Matching direct drive feel"
        },
        {
          "type": "Sticks",
          "brand": "Vic Firth",
          "model": "Vic Firth Matt Garstka Signature",
          "notes": "New signature stick with longer taper for dynamic control",
          "description": "The Madness of Many era saw the debut of Garstka's signature Vic Firth stick, featuring an elongated taper built for the dynamic control his ghost-note vocabulary requires, with balance tailored to traditional grip players."
        }
      ],
      "heads": {
        "bassKick": "Evans EMAD or Remo Powerstroke 3",
        "toms": "Remo Ambassador Coated or Evans G2 Coated",
        "snare": "Remo Ambassador Coated or Evans G1 Coated"
      }
    },
    "playingStyle": {
      "title": "Pushing Metric Modulation Further",
      "content": "\"The Madness of Many\" pushes Garstka's rhythmic vocabulary beyond what he demonstrated on \"The Joy of Motion.\" \"Arithmophobia\" stands as one of his most cited metric modulation showcases — a track that shifts between implied tempos and time feels so fluidly that the underlying math becomes secondary to how naturally the transitions land. \"The Brain Dance\" pairs that modulation with his densest linear independence work to date, each limb tracing a distinct rhythmic path that only resolves into a coherent whole when heard together.\n\nThe technical growth on this album parallels the arrival of his signature gear — a snare and stick pairing built around exactly the dynamic range and control his playing demands. His rudiment-derived linear fills, built from paradiddle-family stickings split between hands and feet (documented on MetalForge's [rudiments technique guide](/technique/rudiments)), reach new levels of speed and clarity here.\n\nFor drummers studying Garstka's development, the jump from \"The Joy of Motion\" to \"The Madness of Many\" is instructive: the core vocabulary — traditional grip, ghost notes, metric modulation — doesn't change, but its execution becomes more confident and more extreme. See [The Joy of Motion drum setup](/articles/the-joy-of-motion-drum-setup) for the preceding chapter and [Parrhesia drum setup](/articles/parrhesia-drum-setup) for where this technique arrives six years later. Full biography at [Matt Garstka's drummer profile](/drummer/matt-garstka).",
      "signatureSongs": [
        {
          "song": "The Brain Dance",
          "album": "The Madness of Many",
          "note": "Technical showcase, densest linear independence to date"
        },
        {
          "song": "Arithmophobia",
          "album": "The Madness of Many",
          "note": "Metric modulation demonstration"
        },
        {
          "song": "Ectogenesis",
          "album": "The Madness of Many",
          "note": "Dynamic use of new cymbal stack textures"
        }
      ]
    },
    "faq": [
      {
        "question": "What drum kit did Matt Garstka use on The Madness of Many?",
        "answer": "Matt Garstka used the same core Pearl Masterworks Maple configuration on The Madness of Many (2016) that he used on The Joy of Motion — a 20x16 inch bass drum, 10x7 and 12x8 inch rack toms, and a 14x14 inch floor tom, all maple shells with MasterCast hoops. The setup remained focused on articulation and dynamic response rather than size."
      },
      {
        "question": "Did Matt Garstka use his signature snare on The Madness of Many?",
        "answer": "The Madness of Many era coincides with the development of the Pearl Matt Garstka Signature Snare — a 14x5 inch maple drum built around a custom bearing edge for his ghost-note-heavy dynamic style. This period also marked the introduction of his Vic Firth signature stick, both representing Garstka's transition from endorsing existing products to co-designing signature gear."
      },
      {
        "question": "What cymbals did Matt Garstka use on The Madness of Many?",
        "answer": "Garstka expanded his Meinl Byzance setup for The Madness of Many, adding a 20 inch Dual Crash and an 18 inch Dual Crash / 16 inch Trash Crash effect stack to the Dual Hi-Hats, Extra Dry Thin Crash, Dual Crash, Sand Ride, and Vintage Trash Hat China he'd used on The Joy of Motion. The 22 inch Sand Ride remained his signature ride voice across both records."
      },
      {
        "question": "When was The Madness of Many released?",
        "answer": "The Madness of Many was released November 11, 2016 on Sumerian Records, Matt Garstka's second full studio album with Animals as Leaders. It followed The Joy of Motion (2014) and preceded Parrhesia (2022)."
      },
      {
        "question": "What makes Arithmophobia a key track for studying Garstka's technique?",
        "answer": "\"Arithmophobia\" from The Madness of Many is widely cited as one of Matt Garstka's clearest metric modulation showcases — the track shifts between implied tempos and rhythmic feels while maintaining a natural, unforced flow, demonstrating the jazz-fusion-informed rhythmic vocabulary that distinguishes his playing from more straightforward djent drumming."
      }
    ],
    "videos": [],
    "relatedAlbums": [
      "the-joy-of-motion-drum-setup",
      "parrhesia-drum-setup",
      "whats-in-matt-garstkas-kit"
    ],
    "relatedDrummers": [
      5,
      14,
      13
    ],
    "relatedArticles": [
      "best-cymbals-for-prog-metal",
      "traditional-grip-in-metal"
    ],
    "conclusion": {
      "title": "The Signature Era Takes Shape",
      "content": "\"The Madness of Many\" captures Matt Garstka at the exact moment his career pivoted from rising star to established voice — technically sharper than \"The Joy of Motion,\" and gear-wise the first album to feature the Pearl and Vic Firth signature products that would carry his name for the rest of his career. The core Masterworks kit stayed constant, but the expanded Byzance cymbal stack and the arrival of his signature snare and sticks mark this as the record where Garstka's setup became fully his own.\n\nFor the album that established this vocabulary, see [The Joy of Motion drum setup](/articles/the-joy-of-motion-drum-setup); for where the signature-era gear matured six years later, see [Parrhesia drum setup](/articles/parrhesia-drum-setup). Full equipment history at [What's In Matt Garstka's Kit](/articles/whats-in-matt-garstkas-kit), and complete biography at [Matt Garstka's drummer profile](/drummer/matt-garstka)."
    }
  },
  "parrhesia-drum-setup": {
    "slug": "parrhesia-drum-setup",
    "albumTitle": "Parrhesia",
    "artist": "Animals as Leaders",
    "drummer": "Matt Garstka",
    "drummerFullName": "Matt Garstka",
    "drummerId": 53,
    "relatedDrummerSlug": "matt-garstka",
    "year": 2022,
    "genre": "Progressive Metal / Djent",
    "label": "Sumerian Records",
    "isAlbumArticle": true,
    "articleType": "album-drum-setup",
    "datePublished": "2026-07-08",
    "dateModified": "2026-07-08",
    "author": "MetalForge Editorial",
    "title": "Parrhesia Drum Setup: Matt Garstka's New DW Custom Shop Rig",
    "description": "Matt Garstka's drum gear on Animals as Leaders' Parrhesia (2022) — the DW Collector's Series custom shop kit he switched to in September 2021, plus his settled Meinl Byzance cymbal stack, behind his most refined recorded performance.",
    "seoKeywords": [
      "parrhesia drum setup",
      "matt garstka parrhesia",
      "animals as leaders 2022 drums",
      "parrhesia drum kit",
      "matt garstka signature snare 2022",
      "monomyth drums",
      "gestaltzerfall drums",
      "animals as leaders drummer gear"
    ],
    "ogImage": "/images/drummers/matt-garstka.webp",
    "intro": {
      "title": "The Signature Setup, Fully Matured",
      "content": "Released March 25, 2022 on Sumerian Records, \"Parrhesia\" is Matt Garstka's third and, to date, most recent album with Animals as Leaders — released six years after \"The Madness of Many\" and following a stretch dominated by touring, clinics, and Garstka's continued growth as one of the most-watched drum educators of his generation. The album's title, Greek for \"frank, fearless speech,\" suits a record that finds the band pushing further into ambient and electronic textures without softening the technical demands on its drummer.\n\nGarstka also switched drum brands ahead of this record: in September 2021 he moved from Pearl to a DW custom shop kit, mixing Collector's Series wood species and shell configurations. Tracks like \"Monomyth\" and \"Gestaltzerfall\" showcase Garstka at his most controlled, now on a new DW Collector's Series build paired with his settled Meinl Byzance cymbal stack and Vic Firth signature sticks.\n\nThis article covers the specific gear and playing approach Garstka brought to \"Parrhesia.\" For the earlier chapters of this gear evolution, see [The Joy of Motion drum setup](/articles/the-joy-of-motion-drum-setup) and [The Madness of Many drum setup](/articles/the-madness-of-many-drum-setup), and for his complete biography, visit [Matt Garstka's drummer profile](/drummer/matt-garstka).",
      "keyPoints": [
        "Released March 25, 2022 on Sumerian Records — Garstka's third Animals as Leaders album",
        "Recorded after six years of touring and educational work between studio albums",
        "Switched from Pearl to a DW Collector's Series custom shop kit in September 2021, ahead of this record",
        "\"Monomyth\" and \"Gestaltzerfall\" showcase Garstka's most controlled, mature dynamic playing",
        "Meinl Byzance cymbal stack and Vic Firth signature sticks carried over unchanged from The Madness of Many"
      ]
    },
    "drumKit": {
      "title": "The New DW Collector's Series Custom Shop Kit",
      "brand": "DW",
      "model": "DW Collector's Series (custom shop)",
      "finish": "Custom per DW custom shop order",
      "config": {
        "shells": "Mixed Collector's Series wood species and shell configurations, per DW custom shop"
      },
      "description": "Ahead of \"Parrhesia,\" Garstka switched drum brands for the first time since joining Animals as Leaders — moving from the Pearl Masterworks Maple kit he'd played since \"The Joy of Motion\" to a DW custom shop build. DW's official September 2021 signing announcement described the kit as mixing Collector's Series wood species and shell configurations, continuing his preference for a compact, articulation-focused setup rather than a larger, louder rig.\n\nSpecific shell sizes for the DW build have not been independently verified; this article will be updated with exact configuration details as sourcing allows. What's confirmed is the brand switch itself and its timing — well ahead of \"Parrhesia\"'s 2022 release — meaning the album's drum tracks were recorded on the new DW kit rather than the Pearl setup documented on \"The Joy of Motion\" and \"The Madness of Many.\"",
      "notes": [
        "Switched from Pearl to a DW custom shop kit in September 2021, ahead of this record",
        "DW's signing announcement describes a mix of Collector's Series wood species and shell configurations",
        "Exact shell sizes for the DW build are not yet independently verified",
        "First drum brand change of Garstka's Animals as Leaders tenure"
      ],
      "estimatedValue": "Not verified (DW Collector's Series custom shop pricing varies by configuration)"
    },
    "snare": {
      "title": "A New DW Collector's Series Snare",
      "brand": "DW",
      "model": "DW Collector's Series Purpleheart",
      "size": "Not verified",
      "shell": "Purpleheart, per DW's Matt Garstka signature drum line",
      "description": "Garstka's Pearl Matt Garstka Signature Snare, used throughout \"The Madness of Many\" era, was retired along with the rest of his Pearl kit when he switched to DW in September 2021. His current primary snare, per DW's Collector's Series signature line, is a Purpleheart-shell model — though DW's custom shop also built him several boutique signature snares (Craviotto, Highwood, Noble & Cooley) that may see use on specific tracks or tours.\n\nExact snare specs and tuning used specifically on \"Parrhesia\" have not been independently verified beyond the brand and model switch; this article will be updated as more detailed sourcing becomes available.",
      "heads": "Not verified for this era",
      "estimatedValue": "Not verified"
    },
    "cymbals": {
      "title": "The Complete Byzance Stack",
      "brand": "Meinl",
      "series": "Meinl Byzance (Multiple lines)",
      "setup": [
        {
          "type": "Hi-Hats",
          "model": "Meinl Byzance 15\" Dual Hi-Hats",
          "position": "Left side",
          "notes": "Complex overtones, dark and washy with clear stick definition"
        },
        {
          "type": "Crash",
          "model": "Meinl Byzance 18\" Extra Dry Thin Crash",
          "position": "Left of hi-hats",
          "notes": "Quick decay, papery attack for fast accents"
        },
        {
          "type": "Crash",
          "model": "Meinl Byzance 19\" Dual Crash",
          "position": "Over rack toms",
          "notes": "Complex overtones for musical crashes"
        },
        {
          "type": "Crash",
          "model": "Meinl Byzance 20\" Dual Crash",
          "position": "Right of toms",
          "notes": "Larger crash for the album's more dramatic moments"
        },
        {
          "type": "Ride",
          "model": "Meinl Byzance 22\" Sand Ride",
          "position": "Far right",
          "notes": "Dry articulation with complex wash — Garstka's constant signature voice since 2014"
        },
        {
          "type": "Stack",
          "model": "Meinl Byzance 18\" Dual Crash + 16\" Trash Crash Stack",
          "position": "Effect position",
          "notes": "Trashy, short accent for modern prog textures"
        },
        {
          "type": "China",
          "model": "Meinl Byzance 18\" Vintage Trash Hat (China)",
          "position": "Far left, high",
          "notes": "Dark, trashy accents without harsh overtones"
        }
      ],
      "description": "The Meinl Byzance stack Garstka used on \"Parrhesia\" is the full, settled version of the setup he'd been assembling since \"The Joy of Motion\" — the same Dual Hi-Hats, Extra Dry Thin Crash, Dual Crashes, Sand Ride, effect stack, and Vintage Trash Hat China that had defined his touring rig since \"The Madness of Many.\" No new pieces were added for this record; instead, \"Parrhesia\" is where the stack gets used with maximum intentionality.\n\nThe 22\" Sand Ride — Garstka's most consistent cymbal choice across all three studio albums — carries even more of the album's atmospheric weight here, its dry, complex wash suiting \"Parrhesia\"'s more ambient production sensibility better than a brighter, more aggressive ride would. The effect stack and China see increased use on tracks that lean into the album's electronic-adjacent textures, giving Garstka short, controlled accents that don't fight with the production's synthesized elements.\n\nAcross three albums and eight years, this Byzance configuration represents one of the most consistent cymbal setups in modern progressive metal — evidence that Garstka's dark, complex cymbal voice was fully formed well before \"Parrhesia,\" and this record simply lets it operate at its most confident.",
      "estimatedValue": "$3,500-4,500 total (Byzance series setup)"
    },
    "hardware": {
      "title": "Hardware and Vic Firth Signature Sticks",
      "items": [
        {
          "type": "Bass Drum Pedal",
          "brand": "Pearl",
          "model": "Pearl Demon Drive Double Pedal",
          "notes": "Last-verified pedal; brand unconfirmed after the September 2021 DW drum/snare switch",
          "description": "No source covering Garstka's September 2021 DW switch specifies a hardware or pedal brand change, so the Pearl Demon Drive is retained here as his last-verified pedal rather than assumed to have carried over unchanged or switched to DW."
        },
        {
          "type": "Hi-Hat Stand",
          "brand": "Pearl",
          "model": "Pearl Demon Drive Hi-Hat Stand",
          "notes": "Last-verified hardware; brand unconfirmed after the September 2021 DW drum/snare switch"
        },
        {
          "type": "Throne",
          "brand": "Pearl",
          "model": "Pearl Roadster D-3500 Throne",
          "notes": "Last-verified hardware; brand unconfirmed after the September 2021 DW drum/snare switch"
        },
        {
          "type": "Sticks",
          "brand": "Vic Firth",
          "model": "Vic Firth Matt Garstka Signature",
          "notes": "Longer taper for dynamic control, fully road-tested by this point",
          "description": "By Parrhesia, Garstka's signature Vic Firth stick had been his primary tool for years, its elongated taper and traditional-grip-oriented balance fully integrated into his touch and dynamic control."
        }
      ],
      "heads": {
        "bassKick": "Evans EMAD or Remo Powerstroke 3",
        "toms": "Remo Ambassador Coated or Evans G2 Coated",
        "snare": "Remo Ambassador Coated or Evans G1 Coated"
      }
    },
    "playingStyle": {
      "title": "Peak-Era Control on Parrhesia",
      "content": "\"Parrhesia\" represents the most controlled version of Matt Garstka's playing captured on record. Where \"The Joy of Motion\" introduced his vocabulary and \"The Madness of Many\" pushed its technical extremes, \"Parrhesia\" is defined by restraint applied with precision — dynamics deployed with the confidence of a drummer who has spent nearly a decade touring and teaching the exact concepts underpinning his own playing.\n\n\"Monomyth\" is frequently cited as the clearest showcase of that dynamic range and feel — ghost notes, ride wash, and full accents deployed with an economy that a decade earlier might have been filled with additional notes. \"Gestaltzerfall,\" widely regarded by fans as Garstka at his technical peak, layers his rudiment-derived linear independence (see MetalForge's [rudiments technique guide](/technique/rudiments) for the underlying vocabulary) against the album's more ambient, electronic-adjacent production without ever losing rhythmic clarity.\n\nThis maturity tracks directly with Garstka's parallel life as an educator — years of breaking down his own concepts for Drumeo and clinic audiences sharpened his ability to deploy them with intention rather than density for its own sake. For the earlier stages of this evolution, see [The Joy of Motion drum setup](/articles/the-joy-of-motion-drum-setup) and [The Madness of Many drum setup](/articles/the-madness-of-many-drum-setup). Full biography and career overview at [Matt Garstka's drummer profile](/drummer/matt-garstka).",
      "signatureSongs": [
        {
          "song": "Monomyth",
          "album": "Parrhesia",
          "note": "Dynamic range and feel"
        },
        {
          "song": "Gestaltzerfall",
          "album": "Parrhesia",
          "note": "Modern Matt at his peak"
        }
      ]
    },
    "faq": [
      {
        "question": "What drum kit did Matt Garstka use on Parrhesia?",
        "answer": "Matt Garstka recorded Parrhesia (2022) on a DW Collector's Series custom shop kit — mixing Collector's Series wood species and shell configurations, per DW's September 2021 signing announcement — having switched from the Pearl Masterworks Maple kit he used on The Joy of Motion (2014) and The Madness of Many (2016)."
      },
      {
        "question": "What snare did Matt Garstka use on Parrhesia?",
        "answer": "Garstka's current primary snare is a DW Collector's Series Purpleheart model, part of the DW custom shop kit he switched to in September 2021, replacing his Pearl Matt Garstka Signature Snare from The Madness of Many era."
      },
      {
        "question": "How does Parrhesia's gear compare to The Madness of Many?",
        "answer": "Parrhesia (2022) marks Garstka's first drum brand change since joining Animals as Leaders: he switched from the Pearl Masterworks Maple kit and Pearl Matt Garstka Signature Snare used on The Madness of Many (2016) to a DW Collector's Series custom shop kit and DW Collector's Series Purpleheart snare in September 2021. His Meinl Byzance cymbal stack and Vic Firth Matt Garstka Signature sticks carried over unchanged."
      },
      {
        "question": "When was Parrhesia released?",
        "answer": "Parrhesia was released March 25, 2022 on Sumerian Records, six years after The Madness of Many (2016). It is Matt Garstka's third studio album with Animals as Leaders and, as of this writing, his most recent."
      },
      {
        "question": "What cymbals does Matt Garstka use on Parrhesia?",
        "answer": "Garstka's Parrhesia cymbal setup is built on Meinl Byzance, including 15 inch Dual Hi-Hats, 18 and 20 inch Dual and Extra Dry crashes, a 22 inch Sand Ride, an 18 inch Dual Crash / 16 inch Trash Crash effect stack, and an 18 inch Vintage Trash Hat China — the same full stack he'd used since The Madness of Many, with the 22 inch Sand Ride remaining his signature ride voice across all three studio albums."
      }
    ],
    "videos": [],
    "relatedAlbums": [
      "the-madness-of-many-drum-setup",
      "the-joy-of-motion-drum-setup",
      "whats-in-matt-garstkas-kit"
    ],
    "relatedDrummers": [
      5,
      14,
      13
    ],
    "relatedArticles": [
      "best-cymbals-for-prog-metal",
      "traditional-grip-in-metal"
    ],
    "conclusion": {
      "title": "A New Kit, the Same Refined Voice",
      "content": "\"Parrhesia\" documents Matt Garstka at maximum confidence on brand-new drums. In September 2021, ahead of this record, he switched from the Pearl Masterworks kit and signature Pearl snare he'd played since \"The Joy of Motion\" to a DW Collector's Series custom shop kit and DW Collector's Series Purpleheart snare — while his Meinl Byzance cymbal stack, anchored by his 22\" Sand Ride, and his Vic Firth signature sticks carried over unchanged. What stayed constant through the gear switch was Garstka himself: eight years of touring, teaching, and refining the same dynamic, jazz-informed vocabulary that first surfaced on \"The Joy of Motion.\"\n\nFor the full arc of that evolution, see [The Joy of Motion drum setup](/articles/the-joy-of-motion-drum-setup) and [The Madness of Many drum setup](/articles/the-madness-of-many-drum-setup). For his complete career-spanning equipment breakdown, visit [What's In Matt Garstka's Kit](/articles/whats-in-matt-garstkas-kit), and for his full biography, see [Matt Garstka's drummer profile](/drummer/matt-garstka)."
    }
  }
};
export default articles;
