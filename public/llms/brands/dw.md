# DW (Drum Workshop) — Metal Drummer Endorsers

DW (Drum Workshop) is an American drum manufacturer founded in 1972 in Oxnard, California. Renowned for handcrafted shells and the industry-standard 9000 series double pedal, DW is the premium choice for drummers who prioritize American-made craftsmanship. The DW 9000 double pedal is one of the most widely used pedals in metal across all subgenres.

## History

**Founded:** 1972 in Santa Monica, California, USA · **Founder:** Don Lombardi

Drum Workshop began as a drum-lesson studio rather than a manufacturer. In 1972, Don Lombardi opened a small teaching space in Santa Monica, California, offering private lessons and workshops under the name "Drum Workshop." A student, John Good, began trading ideas with Lombardi about improving existing drum hardware, and their first product — a height-adjustable throne — sold well enough that Good left his day job to build hardware full-time.

DW's reputation for engineering was cemented in the pedal market: in 1977 the company acquired the tooling behind Camco's well-regarded 5000 pedal and reintroduced an improved version, the DW 5000, in 1979. DW did not sell complete drum kits until much later — after more than fifteen years as a hardware-only company, DW produced its first drum catalog and debuted full kits at the 1989 NAMM show, taking orders for 60 kits on the first day alone. A dedicated Custom Shop opened in Oxnard, California, in 1998.

In the 2010s, DW grew into a broader percussion group, acquiring Latin Percussion, Toca Percussion, and Gibraltar Hardware, and the historic Slingerland Drum Company in 2019. In late 2022, Roland Corporation acquired DW.

**Timeline:**
- 1972 — Don Lombardi founds Drum Workshop as a drum-teaching studio in Santa Monica, California.
- 1977 — DW acquires the pedal tooling behind the Camco 5000 from Camco Drum Co.
- 1979 — DW releases the DW 5000 bass drum pedal, an improved version of the Camco 5000.
- 1989 — DW produces its first drum catalog and debuts complete drum kits at NAMM, taking 60 orders on day one.
- 1998 — DW opens its California Custom Shop in Oxnard with new shell-pressing technology.
- 2019 — DW purchases the historic Slingerland Drum Company.
- 2022 — Roland Corporation acquires DW.

**DW in metal:** DW built its identity on precision hardware and hand-built, timbre-matched shells rather than a dedicated "metal" product line, but that engineering focus made its pedals and hardware a favored choice for drummers who need gear to survive aggressive, high-speed playing. DW's artist roster includes hard rock and metal-adjacent players such as Deen Castronovo and W.A.S.P.'s Aquiles Priester.

Source: [dwdrums.com — The DW Story](https://www.dwdrums.com/the-dw-story/)

## Metal Drummers Who Use DW (Drum Kits)

- George Kollias (Nile) — DW Collectors Series; co-uses Pearl Masterworks for live performances
- Art Cruz (Lamb of God) — Ludwig Drums (current) with DW 9000 hardware; previously DW kits
- Navene Koperweis (Entheos, ex-Animals as Leaders) — DW Collectors Series; DW 9000 Double Pedal
- Bill Ward (Black Sabbath) — Ludwig Classic Maple (current); DW hardware used extensively
- Hannes Grossmann (Obscura, ex-Necrophagist) — DW Collectors Series; DW 9000 Double Pedal

## Metal Drummers Who Use DW Hardware/Pedals

- Ray Luzier (Korn) — DW 9002 Double Pedal; DW 9100 Throne
- Jay Weinberg (Suicidal Tendencies, ex-Slipknot) — DW 9000 Series Double Pedal
- Art Cruz (Lamb of God) — Gibraltar Hardware; previously DW pedals
- Shannon Larkin (Godsmack) — DW 9000 Double Pedal
- Raymond Herrera (Fear Factory) — DW 9000 hardware used alongside Tama kits
- Matt Greiner (August Burns Red) — DW 9000 Double Pedal
- Blake Richardson (Between the Buried and Me) — DW 9000 Double Pedal
- Martin Lopez (Soen, ex-Opeth) — DW hardware
- Travis Orbin (Darkest Hour, ex-Periphery) — DW hardware
- Nicko McBrain (Iron Maiden) — DW 9000 hardware
- Scott Travis (Judas Priest) — DW 9000 Double Pedal
- Mikkey Dee (Scorpions, Motörhead) — DW 9000 hardware
- Frost (Satyricon, 1349) — DW 9000 hardware
- Gavin Harrison (Porcupine Tree) — DW hardware
- Abe Cunningham (Deftones) — DW 9000 hardware
- Hannes Grossmann (Obscura) — DW 9000 Double Pedal
- Isaac Lamb (Kublai Khan TX) — DW 9000 hardware

## Key DW Product Lines for Metal

- Collectors Series — handcrafted shells in virtually any configuration; customizable ply, bearing edge, and finish; DW's flagship drum kit
- Performance Series — production-grade maple shells; more accessible price point while retaining DW quality
- 9000 Series Double Pedal — spring-loaded chain drive; direct and responsive feel; the most commonly spotted DW product in metal touring rigs
- 9002 Double Pedal — longboard variant; favored for extended footboard leverage at high BPM
- 9100AL Throne — air-lift height adjustment; standard touring throne used alongside DW kits and non-DW rigs alike

## FAQ

Q: What DW pedal do metal drummers use?
A: The DW 9000 Series Double Pedal is the most widely used DW product among MetalForge's roster. Ray Luzier uses the DW 9002 variant with its longboard design. Both feature DW's chain-drive mechanism, which delivers direct response for double bass patterns at speed.

Q: What drum kit does Art Cruz use?
A: Art Cruz plays a Ludwig Drums kit (Black Beauty snare) with Trick Pro 1-V double pedals and Gibraltar hardware. He previously used DW kits earlier in his career with Prong.

Q: What is the DW Collectors Series?
A: The DW Collectors Series is DW's hand-built custom drum line, available in exotic and standard wood configurations. Hannes Grossmann and Navene Koperweis use the Collectors Series for its precision manufacturing and tonal customization.

Q: Are DW drums used in death metal?
A: Yes. George Kollias (Nile) and Hannes Grossmann (Obscura) are among the technical death metal drummers who use DW Collectors Series kits. DW's 9000 pedal series is equally prevalent across death metal, thrash, and progressive metal touring rigs.

---

**Brand page:** [/brands/dw](https://metalforge.io/brands/dw)

**More LLM resources:** [Gear by Brand](https://metalforge.io/llms/gear-by-brand.md) · [Gear Guide](https://metalforge.io/llms/gear-guide.md) · [Site index](https://metalforge.io/llms.txt)

*Last updated: 2026-06-19 · Source: [MetalForge.io](https://metalforge.io)*
