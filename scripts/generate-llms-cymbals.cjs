...
const { PILLAR_PAGE, REFERENCE_PAGES, REFERENCE_PAGE_ORDER } = loadModuleConsts(
  path.join(__dirname, '../packages/frontend/data/cymbalReferencePages.js'),
  ['PILLAR_PAGE', 'REFERENCE_PAGES', 'REFERENCE_PAGE_ORDER']
);

// Issue #4737: link the /llms/cymbals/brands/<slug>.md files (generated by
// generate-llms-cymbals-brands.cjs) from this hub — those 4 files existed on
// disk but had zero inbound links from the hub an LLM crawler actually
// starts from.
...
lines.push('## Overview');
lines.push('');
lines.push(PILLAR_PAGE.intro);
lines.push('');
lines.push('## How to Choose Cymbals');
lines.push('');
...
lines.push('');
lines.push(`## Brand Pages (${CYMBAL_BRANDS.length})`);
lines.push('');
for (const brand of CYMBAL_BRANDS) {
  lines.push(`- [${brand.name}](${BASE}/llms/cymbals/brands/${brand.slug}.md) — ${brand.positioning}`);
}
lines.push('');
lines.push('## Best for Metal');
lines.push('');
lines.push(PILLAR_PAGE.bestForMetal);
lines.push('');
lines.push('## FAQ');
lines.push('');
