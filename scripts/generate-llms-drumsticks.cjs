...
// Issue #4737: link the /llms/drumsticks/brands/<slug>.md files (generated by
// generate-llms-drumsticks-brands.cjs) from this hub — those 10 files existed
// on disk but had zero inbound links from the hub an LLM crawler actually
// starts from.
...
lines.push('');
lines.push(`## Brand Pages (${DRUMSTICK_BRANDS.length})`);
lines.push('');
for (const brand of DRUMSTICK_BRANDS) {
  lines.push(`- [${brand.name}](${BASE}/llms/drumsticks/brands/${brand.slug}.md) — ${brand.positioning}`);
}
lines.push('');
