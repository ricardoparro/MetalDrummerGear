...
// Issue #4577: discovery section linking to the per-drummer
// /llms/drumsticks/signature/<slug>.md mirrors (generated by
// scripts/generate-llms-drumsticks-signature.cjs), mirroring how
// public/llms/pedals.md links to its per-drummer setup files.
lines.push(`## Per-Drummer Signature Stick Markdown Files (${count})`);
lines.push('');
lines.push(`Each drummer below also has a dedicated markdown citation file with full specs, an FAQ, and cross-links — the ingestible counterpart to the live [/drumsticks/signature/](${BASE}/drumsticks/signature/) pages.`);
lines.push('');
for (const stick of DRUMSTICKS) {
  const name = drummerNameFor(stick);
  if (!stick.drummerSlug || !name) continue;
  lines.push(`- [${name}'s Signature Sticks](${BASE}/llms/drumsticks/signature/${stick.drummerSlug}.md) — ${stick.brand} ${stick.model}`);
}
lines.push('');
