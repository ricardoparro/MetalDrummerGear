...
// Issue #4577: per-drummer /llms/drumsticks/signature/<slug>.md files
// (generated by scripts/generate-llms-drumsticks-signature.cjs) — one per
// DRUMSTICKS entry with a drummerSlug, mirroring the /llms/pedals.md ->
// /llms/pedals/setups/<slug>.md discovery-link precedent.
const signatureEntries = DRUMSTICKS.filter((stick) => stick.drummerSlug);
lines.push(`## All ${signatureEntries.length} Signature Stick Markdown Files`);
lines.push('');
for (const stick of signatureEntries) {
  const name = drummerNameFor(stick);
  const who = name ? `${name} — ` : '';
  const summary = `${stick.brand} ${stick.model}`;
  lines.push(`- ${who}${summary}: [Markdown](${BASE}/llms/drumsticks/signature/${stick.drummerSlug}.md)`);
}
lines.push('');
lines.push('---');
lines.push('');
