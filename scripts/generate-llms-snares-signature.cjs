 ...
 * /pedals/setups and /cymbals/setups surfaces already have one
 ...
 *    different things.
 *  - packages/frontend/data/snareReferencePages.js (SHELLS_TABLE) — real,
 *    already-published shell-material descriptions used to add setup
 *    context, same discipline as generate-llms-pedals.cjs's drive-type
 *    context.
...
const { SHELLS_TABLE } = loadModuleConsts(
  path.join(__dirname, '../packages/frontend/data/snareReferencePages.js'),
  ['SHELLS_TABLE']
);

function shellMaterialContext(shellMaterial) {
  if (!shellMaterial) return null;
  // shellMaterial can be a compound value like "Maple/Walnut" — match the
  // first token against the table rather than guessing.
  const primary = shellMaterial.split('/')[0].trim();
  const row = SHELLS_TABLE.find((r) => r.material.toLowerCase() === primary.toLowerCase());
  return row ? `**${row.material} shell:** ${row.tone}` : null;
}
...
function buildSignatureMarkdown({ snare, slug, name, band }) {
  ...
  const parts = [];
  parts.push(`# What Snare Does ${name} Use? Signature Model & Specs`);
  parts.push('');
  ...
  const shellCtx = shellMaterialContext(snare.shellMaterial);
  if (shellCtx) {
