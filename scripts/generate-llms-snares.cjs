...
// per SIGNATURE_SNARES entry (SNARES.filter(s => s.isSignature), same filter
// snares.js exports as SIGNATURE_SNARES), generated by
// scripts/generate-llms-snares-signature.cjs. Linked here so the hub file
// surfaces every per-drummer markdown mirror it has.
const signatureSnares = SNARES.filter((snare) => snare.isSignature);
lines.push(`## Signature Snare Markdown Files (${signatureSnares.length})`);
lines.push('');
lines.push(`Per-drummer "what snare does X use" markdown files for the ${signatureSnares.length} drummers with a confirmed signature snare model — full specs, direct answer, and FAQ.`);
lines.push('');
for (const snare of signatureSnares) {
  const name = drummerNameFor(snare);
  const who = name ? `${name} — ` : '';
  lines.push(`- ${who}[${snare.summary}](${BASE}/llms/snares/signature/${snare.drummerSlug}.md)`);
}
lines.push('');
lines.push('---');
lines.push('');
