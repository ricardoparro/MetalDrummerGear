...
const content = fs.readFileSync('./api/drummers/index.js', 'utf8');
...
// Map of drummer names to their Wikipedia URLs for attribution
const originalUrls = {
...
// Extract the drummers array to get id-to-name mapping
...
drummers.forEach(d => {
  ...
  // Replace numeric image path like /images/drummers/1.jpg
  const numericPath = `/images/drummers/${d.id}.jpg`;
  if (d.image === numericPath) {
    const comment = originalUrl ? ` // Original: ${originalUrl}` : '';
    newContent = newContent.replace(
      `image: '${numericPath}'`,
      `image: '${localPath}'${comment}`
    );
    changes++;
    console.log(`Updated ${d.name}: ${numericPath} -> ${localPath}`);
  }
  
  // Replace Wikipedia URL in image field
  if (d.image && d.image.includes('wikipedia')) {
    const comment = ` // Original: ${d.image}`;
    newContent = newContent.replace(
      `image: '${d.image}'`,
      `image: '${localPath}'${comment}`
    );
    changes++;
    console.log(`Updated ${d.name}: Wikipedia URL -> ${localPath}`);
  }
});

// Write the updated file
fs.writeFileSync('./api/drummers/index.js', newContent);
console.log(`\nTotal changes: ${changes}`);
console.log('Updated api/drummers/index.js with local paths');
